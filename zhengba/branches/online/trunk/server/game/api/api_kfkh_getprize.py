...
def doproc(conn, data):
    ...
    # 获取开服狂欢数据
    _kfkhData = g.m.kfkhfun.getKfkhData(uid, where={"day": _day, "hdid": _hdid})
    # 没有数据,无法领取
    if len(_kfkhData) == 0:
        _res["s"] = -2
    ...
    # 未达成条件,无法领取
    if _kfkhData["nval"] < _kfkhData["pval"]:
        _res["s"] = -3
    ...
    # 设置数据
    g.m.kfkhfun.setKfkhData(uid, {"day": _day, "hdid": _hdid}, {"finish": 1})
    # 设置统计数据  方便kfkh排行显示
