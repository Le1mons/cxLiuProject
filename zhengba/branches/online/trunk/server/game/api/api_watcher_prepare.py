...
def doproc(conn, data):
    ...
    _tidList = data[0]
    # 上阵数量有问题
    if len(_tidList) > _con['maxheronum'] or not _tidList:
        _res['s'] = -2
    ...
    # 超过两天了  需要轮回
    if _WatcherData and g.C.NOW() > _WatcherData['rebirthtime']:
        _data = g.m.watcherfun.refreshWatcherData(uid, _WatcherData, _heroList)
...
if __name__ == '__main__':
    uid = g.buid('xuzhao1')
    g.debugConn.uid = uid
    print doproc(g.debugConn, [['5bc0839bc0911a0c4488dcb8']])