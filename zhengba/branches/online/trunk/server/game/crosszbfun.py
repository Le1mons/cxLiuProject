...
def getJifenEnemy(uid, reftype='sys'):
    ...
    for i in xrange(1, 4):
        ...
        if not _uidsArr:
            ...
            _w = {"maxzhanli": {"$gte": _minzl, "$lte": _maxzl}, "uid": {"$nin": [uid]}}
            ...
            _uidsArr = g.crossDB.find("jjcdefhero", _w, fields=['_id','uid'],limit=_uidNeedLen,skip=_skip) #[{uid:1},{uid:2}]
...
# 获取争霸赛前10名排名
def getZBTopRank(uid):
    _dkey = dkey_ZBRank()
    _rankData = g.crossDB.find1('crosszb_zb', {'uid': uid, 'dkey': _dkey})
    if _rankData == None:
        return []
    _data = g.crossDB.find('crosszb_zb', {'dkey': _dkey, 'step': _rankData['step']}, sort=[['rank', 1]], limit=10)
    _res = []
    for d in _data:
        _tmpUid = d['uid']
        _udata = getCrossUserData(_tmpUid)
        if _udata == None:
            continue
        _tmp = {}
        _tmp['uid'] = _tmpUid
        _tmp['rank'] = int(d['rank'])
        _tmp['headdata'] = _udata['headdata']
        _tmp['zhanli'] = _udata['maxzhanli']
        _jjcInfo = g.crossDB.find1('jjcdefhero', {'uid':_tmpUid})
        _tmp['herolist'] = _jjcInfo['fightdata'] if _jjcInfo else []
        _tmp['servername'] = g.m.crosscomfun.getSNameBySid(_udata['sid'])
        _res.append(_tmp)
...
def getZBFightData(uid, isref=0):
    ...
    # for d in _res:
    #     d['info']['userinfo']['servername'] = g.m.crosscomfun.getSNameBySid(d['info']['userinfo']['ext_servername'])
    #     d['head']['servername'] = g.m.crosscomfun.getSNameBySid(d['info']['userinfo']['ext_servername'])
    return _res
...
# 获取跨服争霸排名奖励
def getCrossZBRankPrizeCon(rank):
    _rank = rank
    _con = getCon()
...
def getMyJFRankAndJF(uid):
    ...
    _utc = g.C.UTCNOW()
    _dkey = str(g.C.getWeekNumByTime(_nt))
    _r = g.crossDB.find1("crosszb_jifen", {"uid": uid, "dkey": _dkey})
    _myjifen = 0
