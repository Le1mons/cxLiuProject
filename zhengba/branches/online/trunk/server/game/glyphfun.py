...
def randomBuffVal(gid):
    ...
    # 附加技能
    if _con['extskill'] > 0:
        _res['extskill'] = getRandomExtra('extskill')[0]
    _res['basebuff'] = _res['buff']
    return _res

# 获取随机的额外技能,buff
def getRandomExtra(_type, num=1, recast=0):
    _res = []
    _extCon = g.GC['glyphextra'][_type]
    for i in xrange(num):
        _store = g.C.RANDARR(_extCon['ku'], sum(map(lambda x: x['p'], _extCon['ku'])))
        # 重铸直接随机二号库
        if _type == 'extskill' and recast:
            _store = [i for i in _extCon['ku'] if i['id'] == '2'][0]
...
if __name__ == '__main__':
    print getRandomExtra('extbuff',-1)