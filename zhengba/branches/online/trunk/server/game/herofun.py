...
def getBDSkillBuff(uid, tid):
    ...
    _skill += [_con[_hero['glyph'][x]['extskill']]['skillid'][0] for x in _hero.get('glyph',{}) if 'extskill' in _hero['glyph'][x]]
    _con = g.GC.skill
    for bid in _skill:
        skill = _con[bid]
...
def makeHeroBuff(herodata):
    ...
    if 'weardata' in herodata and '5' in herodata['weardata']:
        _spZhanLi = g.GC['shipin'][herodata['weardata']['5']]['zhanli']

    # 所属职业公会技能战力
    _ghZhanLi = 0
    # 战力 = A1 + B1 + C1 + 其他
    _zhanli = int(_A1 + _B1 + _C1 + _bsZhanLi + _spZhanLi + _ghZhanLi + _kjZhanli+_tyZhanli)
...
def reSetHeroBuff(uid, tid, keys=None):
    _actFun = {
        'baoshi': g.m.baoshifun.getBuff,  # 宝石
        'shipin': g.m.shipinfun.getBuff,  # 饰品
        'equip': g.m.equipfun.getBuff,  # 装备
        'bdskillbuff': getBDSkillBuff,  # 被动buff
        'glyph': g.m.glyphfun.getGlyphBuff,  # 雕纹buff
    }
    _reSetKeys = []
    if keys == None:
    ...
    if 'herobase' not in _reSetKeys:
        ...
        for k in _reSetKeys:
            if k not in _actFun:
                continue
            _key = g.C.STR('herobuff.{1}', k)
            _tmpBuff = _actFun[k](uid, tid)
...
# 重算后是否写入数据库
def _reSetHerosBuff(uid, herolist, isset=1):
...
def updateByUserWear(uid, herotid, _type='', eqid=''):
    ...
    # 穿戴普通装备
        ...
        if _equipInfo:
            ...
            del _equipInfo['_id']
            # 检查装备使用数量
            g.m.equipfun.chkEquipUsenum(uid, _equipInfo)
...
def takeOffUserWear(uid, herotid, _type, eqid):
    ...
    if _type == '5':
        ...
        g.mdb.update('equiplist', {"_id": _eqInfo['_id']}, {'usenum': _eqInfo['usenum']})
...
def ChkHero(_con, heroinfo, herolist, ishecheng=1):
    #heroinfo = 目标英雄的配置
    ...
    for i in herolist:
        ...
        # 升星判断
            ...
            for condDict in _con['delhero']:
                # 星级相同 并且（种族随便或者 种族相同）
                if condDict.get('star') == i['star'] and (not condDict.get('samezhongzu') or heroinfo['zhongzu'] == i['zhongzu']):
                    _delHeros.append(i)
                    break
                elif condDict.get('sxhero') and condDict.get('num') != len(_chkHeros):
    ...
    #只判断总数不够
    if len(_chkHeros + _delHeros) == len(herolist):
        _res['res'] = True
...
def onHeroTongyu(uid, hid, star=None):
    ...
    # 以后增加11星 也是6级
    if not star:
        _tyLV = _star - 4 if _star <= 10 else 6
    else:
        _tyLV = star - 4 if star <= 10 else 6
...
def getDestinyData(num):
    _res = {'buff': [], 'prize': [], 'avater': '','zhanli':0}
...
if __name__ == '__main__':
    uid = g.buid('xuzhao')
    # reSetHeroBuff('0_5c27440ae1382345d0a4b907','5c2b5fade13823565da57f83')
    reSetHeroBuff(uid, '5c29c0b7c0911a34646bae77')