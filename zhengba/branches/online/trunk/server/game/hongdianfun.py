...
def getAllUnreadEmail(uid):
    ...
    _nt = g.C.NOW()
    _where = {"uid": uid, "$or": [{"isread": 0}, {"getprize": 0}], 'passtime': {'$gte': _nt}}
    _emails = g.mdb.find('email', _where,fields=['_id','senduid','prize'])
    for i in _emails:
        if 'senduid' in i and _res['chat'] == 0:
            _res['chat'] = 1
        elif 'prize' in i and _res['system'] == 0:
            _res['system'] = 1
...
def getUnGetDengjiPrize(uid, hdid):
    ...
    # 已经购买  达到指定的等级，但是没有领取
    if -1 in _valInfo['gotarr'] and notagetarr:
        _val = 1
...
def getUngotLeijichongzhi(uid,_hdInfos):
    hongDianHDIDS = []
...
def getAllHongdian(uid, types=[], iscache=1):
    ...
    _key = 'yueka_xiao'  # 小月卡
    if _all or _key in types:
        _rData[_key] = getUnGetYueKa(uid, 'xiao')

    _key = 'yueka_da'  # 大月卡
    if _all or _key in types:
        _rData[_key] = getUnGetYueKa(uid, 'da')
    ...
    if _all or _key in types:
        ...
        specHuoDong = g.m.huodongfun.getIDSByHypes([14,13,11])
        _htype14List = []
        _htype13List = []
        _htype11List = []
        for __hinfo in specHuoDong:
            ...
            if __hinfo['htype'] == 13:
                _htype13List.append( __hinfo )
            if __hinfo['htype'] == 11:
                _htype11List.append( __hinfo )          
        
        # 周常活动红点
        if len(_htype14List)>0 : hdhuodong += getZChuodong(uid,_htype14List)
        #累计充值活动
        if len(_htype13List)>0 : hdhuodong += getUngotLeijichongzhi(uid, _htype13List)
        #积天返利
        if len(_htype11List)>0 : hdhuodong += getUngotJitian(uid,_htype11List)
        #---------//----------
        _rData[_key] = hdhuodong
    ...
    _key = 'monthfund'
    if _all or _key in types:
        _rData[_key] = getMonthFundHD(uid)
...
def getXszmHD(uid):
    ...
    # 充值金额满足某一档
    for idx, ele in enumerate(g.GC['xianshi_zhaomu']['jifen2prize']):
...
def getCompetingHD(uid):
    ...
    # 周一到周五且未报名
    if g.chkOpenCond(uid, 'ghcompeting') and g.C.WEEK != 6 and gud['ghpower'] < 3 and g.m.competingfun.isCanSignUpGame(gud['ghid']):
        return 1
    ...
    if g.crossDB.find1('competing_main', {'season': _season,'ghid':gud['ghid']}, fields={'_id':1}) is not None:
        if g.C.ZERO(_nt) + _con['time']['fight'][0] <= _nt <= g.C.ZERO(_nt) + _con['time']['fight'][1] and (
                g.m.competingfun.getCanFightNum(uid) > 0 and not g.getAttrByDate(uid, {'ctype': 'ghcompeting_openonce'})
        ):
...
def getMRSChongdian(uid):
    ...
    if not _scData or 'receive' in _scData[0]:
        return 0

    if _scData[0]['v'] < g.GC['meirishouchong']['val']:
        return 0
...
def getMonthFundHD(uid):
    ...
    resHDInfo = monthfund.getMonthFundInfo(uid)

    resHDDict = {str(hd['hdid']):{'isbuy':0} for hd in resHDInfo}
    ...
    for _tmpData in _userData:
        ...
        if _nowDay not in _tmpData['gotarr']:
            ...
            resDict.update({str(hdid):{'canlq':1,'isbuy':isbuy}})
...
def getGuildHD(uid):
    ...
    _res['fuben'] = _maxNum - g.m.gonghuifun.getPkNum(uid)

    # 公会探宝
    _where = {'uid': uid, '$or': [{'treasure.boss': {'$exists': 1}}, {'treasure.freetime': {'$gt': g.C.NOW()}}]}
    # 可以探宝搜寻时
    if g.chkOpenCond(uid, 'friendhelp') and not g.mdb.find1('friend', _where):
        _res['treasure'] = 1

    # 公会争锋
    _res['competing'] = getCompetingHD(uid)
    # 团队任务
    if g.mdb.find1('gonghuiattr', {'ghid': _guildID, 'ctype': 'teamtask_leader','ispass':{'$exists':0}}, sort=[['k', -1]], fields={'_id':1}) and g.m.teamtaskfun.getFightNum(uid) > 0:
        _res['teamtask'] = 1
    return _res
...
def getFashitaHD(uid):
    ...
    _nt = g.C.NOW()
    if not _con[0] + g.C.ZERO(_nt) <= _nt <= _con[1] + g.C.ZERO(_nt) or not g.chkOpenCond(uid, 'temple_devil'):
        return _res
...
def getFriendHD(uid):
    ...
    if len(_filterList) >= _max:
        return _res

    if g.m.friendfun.getCanReceiveYinji(uid):
        _res = 1
    return _res
...
def getCanSCJJ(uid):
    ...
    if len(_canprizelist) > len(_recList):_ret = 1

    return _ret

if __name__ == "__main__":
    uid = g.buid('xuzhao')
    # print getShizijunHD(uid)
    #a = g.m.huodongfun.getHongDian(uid)
    a = getXszmHD(uid)
    print a

    # {u'finish': 0, u'uid': u'0_5aec54eb625aee6374e25e0c', u'nval': 1785, u'pval': 20, u'htype': 18, u'tab': 3, u'hdid': 9, u'lasttime': 1532688288, u'day': 4, u'ctime': 1532688288}
