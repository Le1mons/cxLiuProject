...
def getOpenData(uid, hdid):
    ...
    _valInfo = getUserHuoDong(uid, {'hdid': hdid, }, fields=['val', 'gotarr', 'mfetime', 'mfstime', 'usetime', 'isbuy','lqval'])
    # 月卡和至尊卡数据
...
def getPrize(uid, hdid):
    ...
    _userData = getUserHuoDong(uid, {'hdid': hdid, }, fields=['val', 'gotarr', 'mfetime', 'mfstime', 'daytime', 'htype','lqval'])
    # 检查是否可以领取奖励
    if len(_userData) <= 0:
        # 没有数据
        _res['s'] = -1
...
def hdEvent(uid, hdinfo, etype, *args, **kwargs):
    ...
    _data = {
        ...
        'lqval': 0,      # 领取天数
        'gotarr': [],
        'usetime': g.C.getDate(_ntZero, "%m月%d日")+ '-'+ g.C.getDate(_etime, "%m月%d日"),
        'isbuy': 1,
        'daytime': _nt
    }
    # 保存数据
    g.m.huodongfun.setMyHuodongData(uid, _hdid, _data)
...
def getMonthFundInfo(uid, keys=None):
    ...
    _w = {"stime": {"$lte": _nt}, '$or': [{"etime": {"$gte": _nt}, 'htype': {'$in': _monthFundHtype}}]}
...
# g.event.on("firstlogin", onFirstLogin)
# getFundShow
def getFundShow(uid):
    # 小基金 大基金
