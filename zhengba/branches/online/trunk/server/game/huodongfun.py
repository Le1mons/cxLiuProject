...
def getOpenList(uid=None, isid=0, field=None, isrtime=0, isdouble=0, hdvtype="", isxianshi=1):
    ...
    hdfields = ['_id', 'showtime', 'hdid', 'name', 'img', 'arg', 'showitem',
                ...
                'order', 'btntype', 'data', 'icon', 'model', 'buff', 'tips']
    if uid:
        gud = g.getGud(uid)
        exTime = gud['ctime']
        czt = g.C.ZERO(exTime)
        pDay = (nt - czt) // (24 * 3600)
        hdwhere = {'$or': [hdwhere, {'ttype': 1, 'stime': {'$lte': pDay}, 'etime': {'$gt': pDay}}]}
...
def getCanFinishHD(uid, hdid=None):
    ...
    if hdid:
        ...
        if not openHd:
            openHd = getOpenList(uid, field=['data', 'htype', 'hdid', 'rtime', 'etime', 'arg', 'stime'])
    ...
    for hd in openHd:
        ...
        if fhdid:
            canFinish.append(hd['hdid'])
    return canFinish


# 活动事件
def event(uid, etype, *args, **kwargs):
    _mcKey = 'huodongevent_hdlist_event'
    # 获取活动列表
...
def timer_sendPrize(_htype, iszchd=True, cond=None):
    ...
    for x in _info:
        ...
        _rank = g.mdb.find('hddata', _where, sort=[['val', -1], ['lasttime', 1]], limit=1000)
...
def setZCHDval(uid, mark, val=1):
    ...
    if _hdInfo:
        ...
        _hdid = _hdInfo['hdid']
        # 需要记录战力
        _zhanli = g.m.zypkjjcfun.getDefendHeroZhanli(uid)
        _setData = {'$inc': {"val": val}, '$set': {'zhanli': _zhanli, 'lasttime': _nt}}
