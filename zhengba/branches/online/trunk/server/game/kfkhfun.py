...
def getCondVal(uid, day, hdid, defval=0):
    ...
    # 法师塔通关
        ...
        if not _fashitaInfo:
            ...
            _passlist = _fashitaInfo.get('passlist')
            _retVal = _passlist[-1]
...
def onKFKH(uid, htype, day, val=1, cond=0,*args, **kwargs):
    ...
    # 探险通关
        ...
        setKfkhData(uid, {'htype': 11, 'day': 1}, {'nval': val})

    # 友情印记赠送
    elif htype == 12:
        setKfkhData(uid, {'day': 1, 'htype': 12}, {'$inc': {'nval': val}})

    # 法师塔通关
    elif htype == 13:
        setKfkhData(uid, {'day': 2, 'htype': 13}, {'nval': val})

    # 祭坛招募
    elif htype == 14:
        setKfkhData(uid, {'htype': 14, 'day': 2}, {'$inc': {'nval': val}})

    # 竞技场最高分
    elif htype == 15:
        setKfkhData(uid, {'nval': {'$lt': val}, 'htype': 15,'day':day}, {'nval': val})

    # # 英雄升级
    elif htype == 16:
        _con = g.GC['kaifukuanghuan']['3']
        _hdidList = [int(k) for k, v in _con.items() if v['htype'] == 16 and v['cond'][0] == cond]
        if _hdidList:
            setKfkhData(uid, {'htype': 16, 'hdid': {"$in": _hdidList}}, {'$inc': {'nval': 1}})

    # 悬赏任务完成
    elif htype == 17:
        _con = g.GC['kaifukuanghuan']['4']
        _hdidList = [int(k) for k, v in _con.items() if v['htype'] == 17 and v['cond'][0] <= cond]
        setKfkhData(uid, {'htype': 17, 'hdid': {"$in": _hdidList}}, {'$inc': {'nval': 1}})

    # 悬赏任务刷新
    elif htype == 18:
        setKfkhData(uid, {'htype': 18, 'day': day}, {'$inc': {'nval': 1}})

    # 十字军战斗胜利
    elif htype == 19:
        setKfkhData(uid, {'htype': 19, 'day': day, 'nval': {'$lt': val}}, {'nval': val})

    # 获得X星英雄
    elif htype == 20:
        _con = g.GC['kaifukuanghuan']['5']
        _hdidList = [int(k) for k, v in _con.items() if v['htype'] == 20 and v['cond'][0] == cond]
        setKfkhData(uid, {'htype': 20, 'hdid': {"$in": _hdidList},'day':day}, {"$inc": {'nval': 1}})

    # 英雄宝石达到X色X星
    elif htype == 21:
        _con = g.GC['kaifukuanghuan']['6']
        _hdidList = [int(k) for k, v in _con.items() if v['htype'] == 21 and v['cond'][0] == cond]
        _w = {'baoshilv': {'$gte': cond}, 'uid': uid}
        _retVal = g.mdb.count('hero', _w)
        setKfkhData(uid,{'htype':htype,'day':6,'hdid':{'$in':_hdidList}, 'nval':{'$lt':_retVal}},{'nval':_retVal})

    # 许愿池抽奖
    elif htype == 22:
        setKfkhData(uid, {"htype": 22, 'day': day}, {'$inc': {'nval': val}})

    # 合成N星英雄
    elif htype == 23:
        _con = g.GC['kaifukuanghuan']['7']
        _hdidList = [int(k) for k, v in _con.items() if v['htype'] == 23 and v['cond'][0] == cond]
        setKfkhData(uid, {'htype': 23, 'day': day,'hdid':{'$in':_hdidList}}, {"$inc": {'nval': 1}})

    # 玩家达到多少级
    elif htype == 24:
        setKfkhData(uid,{'htype':htype,'day':7},{'nval':val})

    # 监测红点
    _data = getKfkhData(uid, {'htype': htype, 'day': day, 'finish': 0})
    for i in _data:
        if i['nval']  >= i['pval']:
