...
def getMaxMapidRank(iscache=1, uid=None):
    ...
    if iscache and _cacheRank != None:
        ...
        if len(_rankList) > 0:
            g.mc.set("SHILIRANK", {"list": _rankList, 'uid2rank': _uid2rank}, 20)
...
def getArenaRank(iscache=0, uid=None, iszhanli=False):
    ...
    for ele in _rInfo:
        ...
        # if len(_rankList) > 0:
        #     g.mc.set("ZypkjjcRank", {"list": _rankList, 'uid2rank': _uid2rank}, 20)
...
def getChampionTrialRank(iscache=0, uid=None):
    ...
    for ele in _rInfo:
        ...
        # if len(_rankList) > 0:
        #     g.mc.set("ChampionTrialRank", {"list": _rankList, 'uid2rank': _uid2rank}, 20)
    ...
    if uid != None and _arenaInfo:
        ...
        if uid in _uid2rank:
            ...
            _myRank = g.mdb.count('championtrial', {'jifen': {'$gte': _jifen}})
            if _myRank >= 1000: _myRank = -1
...
def getDJDRRank(iscache=0, uid=None):
    ...
    if iscache and _cacheRank:
        ...
        htype = 14
        _hdInfo = g.mdb.find1('hdinfo', {'htype': htype,'rtime':{'$gte': g.C.NOW()},'data.mark': 'dianjin','stime':{'$lte': g.C.NOW()}})
        if _hdInfo:
            hdid = _hdInfo['hdid']
            _info = g.mdb.find('hddata', {'hdid': hdid,'val':{'$gt': 0}},sort=[['val',-1],['lasttime',1]],limit=50,fields=['_id','uid','val'])
...
def getYZTSRank(iscache=0, uid=None):
    ...
    if iscache and _cacheRank:
        ...
        htype = 14
        _hdInfo = g.mdb.find1('hdinfo', {'htype': htype, 'rtime': {'$gte': g.C.NOW()}, 'data.mark': 'shizijun','stime':{'$lte': g.C.NOW()}})
        if _hdInfo:
            hdid = _hdInfo['hdid']
            _info = g.mdb.find('hddata', {'hdid': hdid,'val':{'$gt': 0}}, sort=[['val', -1], ['lasttime', 1]], limit=50,fields=['_id','uid','val'])
            # _info.sort(key=lambda x: (x['val'], g.getGud(x['uid'])['maxzhanli']), reverse=True)
...
def getSJQBRank(iscache=0, uid=None):
    ...
    if iscache and _cacheRank:
        ...
        htype = 14
        _hdInfo = g.mdb.find1('hdinfo', {'htype': htype, 'rtime': {'$gte': g.C.NOW()}, 'data.mark': 'xstask','stime':{'$lte': g.C.NOW()}})
        if _hdInfo:
            hdid = _hdInfo['hdid']
            _info = g.mdb.find('hddata', {'hdid': hdid,'val':{'$gt': 0}}, sort=[['val', -1],['lasttime', 1]], limit=50,fields=['_id','uid','val'])
            # _info.sort(key=lambda x: (x['val'], g.getGud(x['uid'])['maxzhanli']), reverse=True)
...
# 获取公会榜排名
def getGuildRank(iscache=0, uid=None):
    _rankList = []
    _name2rank = {}
    ...
    if uid != None and _myGuild:
        _lv = _myGuild['lv']
        if uid in _name2rank:
            _guildName = _myGuild['name']
            _myRank = _name2rank[_guildName]
        else:
            _myRank = g.mdb.count('gonghui', {'lv': {'$gte': _lv},'exp':{'$gte':_myGuild['exp']}})
            if _myRank >= 1000: _myRank = -1
...
def getKFZBRank(iscache=0, uid=None):
    ...
    if iscache and _cacheRank:
        ...
        for rank,ele in enumerate(_List):
            ...
            temp['headdata']['svrname'] = g.m.crosscomfun.getSNameBySid(ele['sid'])
...
def getTXXFRank(iscache=0, uid=None):
    ...
    if iscache and _cacheRank:
        ...
        htype = 14
        _hdInfo = g.mdb.find1('hdinfo', {'htype': htype, 'rtime': {'$gte': g.C.NOW()}, 'data.mark': 'tanxian','stime':{'$lte': g.C.NOW()}})
        if _hdInfo:
            hdid = _hdInfo['hdid']
            _info = g.mdb.find('hddata', {'hdid': hdid}, sort=[['val', -1],['lasttime', 1]], limit=50,fields=['_id','uid','val'])
            # _info.sort(key=lambda x: (x['val'], g.getGud(x['uid'])['maxzhanli']), reverse=True)
            _myRank = -1
            for idx, i in enumerate(_info):
                _temp = {}
...
def getGuildCompetingDivisionRank(iscache=0, uid=None):
    ...
    _competingInfo = g.crossDB.find1('competing_main',{'season':_season,'ghid':_ghid},fields=['_id','jifen','division','segment','group','season','pre_seg'])
    if not _competingInfo:
        return

    _segment = _competingInfo['segment'] if 6 <= g.C.HOUR() < 22 else _competingInfo.get('pre_seg', 1)
    ...
    if iscache and _cacheRank != None:
        ...
        if g.C.HOUR() >= 22 or g.C.HOUR() < 6:
            del _w['segment']
            _w.update({'pre_seg': _competingInfo.get('pre_seg', 1)})
...
def getTempleDevilRank(iscache=0, uid=None):
    ...
    if uid in _uid2rank:
        ...
        _myRank = g.mdb.count('gameattr',{'ctype':'temple_devil_dps', 'v': {'$gte': _myDps['v']},'lasttime':{'$gte':_zt}}) + 1
