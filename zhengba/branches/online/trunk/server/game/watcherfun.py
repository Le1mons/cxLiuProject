...
# 刷新的数据 层数倒退30
def refreshWatcherData(uid, data, heroList):
...
def getFightPrize(uid, _data, _prizeDict):
    ...
    # 如果是合剂 立马使用 增加对应的buff属性
        ...
        # 继承buffti
        _setData['herolist'] = _data['herolist'] = g.m.watcherfun.useMixture(_data['herolist'], {_mixtureId: 1})
...
def getHeroInfo(uid, tidlist):
    ...
    for _tid in tidlist:
        # 默认站在后排
        _chkFightData = g.m.fightfun.chkFightData(uid, {'4':_tid})
