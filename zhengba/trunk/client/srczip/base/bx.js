;(function() {
    ...
    * */
    X.timeout = function(node,toTime,endcall,stepcall,conf,timeCall){
        ...
        var setText = function () {
            ...
            if (conf.showDay && _leftSeconds > 24 * 3600) {
                var _seconds = _leftSeconds;
                var dayTime = 24 * 3600;
                var day = parseInt(_leftSeconds / dayTime);
                if (day > 0) _seconds = _leftSeconds - day * dayTime;
                if (day > 0) {
                    if(conf.onlyDay){

                        _leftStr = (day*1+1)+ L("tian") ;
                    }else{
                        _leftStr = day + L("tian") + X.timeLeft(_seconds);
            ...
            stepcall && stepcall(_leftStr,_leftSeconds);
    ...
    X.timeout1 = function(target,node,v,endcall,stepcall,intr1,intr2){
        if (!v || v <= 0) return;
        var n = v;
        var i = n;
    ...
    //将一个时间戳转换为富文本格式
    X.timePanel = function(node,toTime,size,type,callback,args){
        size = size || 18;
        type = type || "left";//左对齐还是右对齐
        args = args || {};
        desc = args.desc || L("HUODONGLEFT");
        var anchor;
        var pos;
        if(type == "left"){
            anchor = {x: 0, y: 0.5};
            ...
            anchor = {x: 1, y: 0.5};
            pos = {x: node.width, y: node.height / 2};
        }else if(type == "center"){
            anchor = {x: 0.5, y: 0.5};
        ...
        (function (node,desc) {
            X.timeout(node, toTime, function () {
                ...
                if (seconds >= seconds_day) {
                    ...
                    var str1 = X.STR(L('DJS_DAY'), day, X.timetostr(toTime+(seconds - seconds_day * day), 'h:mm:s'));
                    var str2 = desc + X.STR(L("HUODONGTIME") ,"#77ef6b", str1);
                    X.setRichText({
                        str: str2,
                        parent: node,
                        size:size,
                    ...
                    var str1 = desc + X.STR(L("HUODONGTIME") ,"#77ef6b", str);
                    // me.nodes.panel_sj.children[0].text(str1);
                    X.setRichText({
                        str: str1,
                        parent: node,
                        size:size,
    ...
    */
    ...
    X.radio = function(btns,onChange,obj,onTouch){
        ...
        for(var i=0;i<btns.length;i++){
            ...
            btns[i].touch(function(sender,type,fromwhere){
                if (type == ccui.Widget.TOUCH_BEGAN && !sender.disabledCall){
                      var txt = getText(sender);
                    !onTxtColor && txt && txt.setTextColor && txt.setTextColor(C.color(color1));
                    onTouch && onTouch(sender);
                }

                if ((type == ccui.Widget.TOUCH_CANCELED || type == ccui.Widget.TOUCH_MOVED) && sender.isBright() && !sender.disabledCall){
                    var txt = getText(sender);
                ...
                if(type==ccui.Widget.TOUCH_ENDED){
                    ...
                    if(sender.disabledCall){
                        sender.disabledCall && sender.disabledCall(sender);
                        return;
                    }
                    if (fromwhere!='fromcode' && !sender.isBright()){
                        return;
    ...
    X.autoInitUI  = function(root,saveTo){
        ...
        X.forEachChild(root,function(node){
            var _name = node.getName();
            if(_name && (_name.toString()).substr(-1)=='$') {
                var _key = _name.substr(0, _name.length-1);
