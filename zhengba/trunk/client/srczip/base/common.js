...
//  that to be increased. Set to zero for unlimited.
    ...
    EventEmitter.prototype.once = function(type, listener,caller) {
        ...
        if (type == "paysuccess") {
            //外网点充值会监听"paysuccess"事件 如果调了充值SDK而不充值的话事件会保留 所以每次加“paysuccess”监听时
            //清除其他所有没有触发的监听
            this.removeAllListeners("paysuccess");
        }
        var fired = false;
        var that = this;
        function g() {
...
;(function(global){
    ...
    ccui.Widget.prototype.touch = function(fun,caller,conf){
        ...
        this._touchFunction = function(sender,type,fromwhere){
            ...
            if (type == ccui.Widget.TOUCH_BEGAN) {
				...
				X.tsgamelog("touch",{"name":sender.getName(),"pos":sender.convertToWorldSpace( sender.getTouchBeganPosition() )});
    ...
    ccui.Widget.prototype.noMove = function(fun,touchDelay){
        this.touch(function(sender,type){
    ...
    ccui.ListView.prototype.removeItemByTag = function(item) {
        var index;
        ...
        if(index != undefined) return this.removeItem(index);
    };
    if (!cc.sys.isNative) {
        cc.Sprite.prototype.initWithSpriteFrameName = function (spriteFrameName) {
            cc.assert(spriteFrameName, cc._LogInfos.Sprite_initWithSpriteFrameName);
            var frame = cc.spriteFrameCache.getSpriteFrame(spriteFrameName);
            if (!frame) return cc.error('图片缺失：', spriteFrameName);
