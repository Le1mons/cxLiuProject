(function() {
    ...
    G.class.hero = {
        ...
        getHeroByPinglun: function(id) {
            var arr = [];
            var heroArr = this.get();
            for(var key in heroArr){
                if(heroArr[key].pinglunid == id){
                    arr.push(heroArr[key]);
                }
            };
        ...
        getSkillOne: function(idx, hid, dengjielv) {
            ...
            if (idx == 0) {
                ...
                var skillArr = skill && skill[X.STR('bd{1}skill', idx)];
                return {
                    idx: idx,
                    ...
                    skillArr: skillArr ? skillArr : conf[X.STR('bd{1}skill', idx)]
                };
            }
        },
        ...
        getMaxStar: function (hid) {
            if (G.gc.herostarup[hid]) {
                return 6 + Object.keys(G.gc.herostarup[hid]).length;
            } else {
                return G.gc.hero[hid].star;
            }
        },
        // 获取可以升级的技能
        ...
        getModel: function(data) {
            ...
            if (G.gc.skin[data.model]) return data.model;
        ...
        getSkinId: function (hid) {
            var arr;
            var conf = G.gc.skin;

            for (var i in conf) {
                if (X.inArray(conf[i].hid, hid)) {
                    if (!arr) arr = [];
                    arr.push(i);
                }
            }
        ...
        getSoundByHid: function (hid, skin) {
            var conf = G.gc.hero[hid];

            if(!conf) return console.log("no hero config" + conf);

            if(!conf.voice) return console.log("hero no sound");

            var soundName = X.arrayRand(conf.voice);

            if (skin && G.gc.skin[skin.sid]) soundName = X.arrayRand(G.gc.skin[skin.sid].sound);

            X.audio.stopEffectByType("hero");
            if(G.gc.ifChangeModels()) {
                var arr = ["34025", "34026", "3402a"];
                if(X.inArray(arr, soundName)) soundName += "_2";
            }
            cc.log("play hero sound ++++++++++ id: " + soundName);
            X.audio.playEffect("sound/" + soundName + ".mp3", false, "hero");
        }
    };
    ...
    G.class.herostarup = {
        ...
        getUpStarNum: function (hid) {
            var star = 0;
            var me = this;
            var str = hid.toString();
            var id = str.substring(0, str.length - 1);
            var curStar = str.substring(str.length - 1, str.length);

            for (var i = curStar * 1; i <= 6; i ++) {
                if(G.gc.hero[id + i]) star = i;
            }

            if(G.gc.hero[id + 6]) {
                var conf = me.getById(id + 6);
    ...
    G.class.equip = {
        ...
        getFMMaxLv: function () {
            ...
            return X.keysOfObject(G.gc.zbfm.base.common).length;
        },
        getFMBuff: function (lv, idx) {

            return G.gc.zbfm.base.common[lv][idx];
        }
    };
    //NPC
    G.class.npc = {
    ...
    G.class.wenwu = {
        get: function() {
            return G.class.getConf('wenwuinfo');
        },
        getById: function(id) {
            var me = this;
            return me.get()[id];
    ...
    G.class.tanxian = {
        ...
        getHhPrize: function (passArr) {
            ...
            for (var i = 0; i < conf.length; i ++) {
                if(!X.inArray(passArr, i) && conf[i][2]) {
                    obj.con = conf[i];
                    obj.idx = i;
                    break;
                }
    ...
    G.class.meirishilian = {
        ...
        getSdIndex: function (type) {
            var conf = this.getConfByType(type);
            var index = 0;
            for (var i in conf) {
                if (P.gud.lv >= conf[i].openlv * 1) index ++;
            }
    ...
    G.class.opencond = {
        ...
        getIsOpenById: function(id, obj) {
            ...
            if (conf['main']) {
                ...
                for (var i = 0; i < cond.length; i++) {
                    var co = cond[i];
                    if(co[0] == "severday"){//判断开区条件
                        if(G.time < G.OPENTIME + X.getOpenTimeToNight() + (co[1] - 2) * 24 * 3600){
                            canMain = false;
                            break;
                        }
                    } else if (obj[co[0]] < co[1]) {
                        canMain = false;
                        break;
                    }
                }
        ...
        getLvById: function(id) {
            ...
            var conf = this.getCondById(id);
            if (Object.keys(conf).length == 0) return 1;
            var arr = [];
            if (conf.optional) {
    ...
    G.class.friend = {
        ...
        getPrizeByLv: function (lv, isJy) {
            var me = this;
            var conf = me.get().base[isJy];
            var keys = X.keysOfObject(conf);
    ...
    G.class.midautumn = {
        get: function() {
            return G.class.getConf('midautumn');
        },
        getById: function(id) {
            return this.get()[id];
        },
        getDrop: function() {
            var drop = this.getById('drop');
            var result = [];
        ...
        getTask: function(){
            var conf = this.getById('toTheMoon').task;
            var keys = X.keysOfObject(conf);
            keys.sort(function(a, b){
                return a*1 - b*1;
            });
    ...
    G.class.newyear = {
        get: function() {
            return G.class.getConf('newyear');
        },
        getById: function(id) {
            return this.get()[id];
        },
    ...
    G.class.shenqi = {
        ...
        getUpLvNeedByLv: function (lv, type) {
            return G.class.getConf("shenqicom").base.lvneed[type][lv];
        },
        getUpDjNeedByDj: function (denjie, type) {
            return G.class.getConf("shenqicom").base.dengjieneed[type][denjie];
        },
        getTaskById: function (id) {
            return G.class.getConf("shenqitask")[id];
        },
        getSqJxDps: function (id, jxlv) {
            var waketype = G.gc.shenqicom.shenqi[id].waketype;
            var conf = G.gc.shenqicom.base.wake[waketype][jxlv || 0];
            return conf.shenqidpspro;
        }
    };
    //秘境
    G.class.watchercom = {
    ...
    G.class.slzt = {
        ...
        getCom: function () {
            return G.gc.shilianztcom
        },
        getGL: function (id) {
            return this.getCom().mirrorpro[id +1];
        },
        getBuff: function () {
            return G.gc.shilianztbuff
        },
        getDay: function () {
            return this.getCom().day;
             
        },
        getTask: function () {
            var conf = this.getCom().task;
            return conf 
        },
        getfoevertask: function () {
            var conf = this.getCom().foevertask;
            return conf 
        },
        getEvent: function () {
            var conf = this.getCom().event;
            return conf 
        },
        getLayerinfo: function () {
            var conf = this.getCom().layerinfo;
            return conf 
        },
        getQuestions: function () {
            var conf = this.getCom().questions;
            return conf 
        },
    };
    G.class.szn = {
        get: function () {
            return G.gc.sanzhounian   
        },
        getjfConf: function () {
            return this.get().jifenprize
             
        },
        getduiHuan: function () {
            
            return this.get().duihuan;
        },
        getqdTask: function () {
            return this.get().task
             
        },
        getTarget: function (layer) {
            return this.get().targetprize[layer]; 
        },
        getAllTarget: function (layer) {
            return this.get().targetprize; 
        },
        getlotteryinfo: function () {
            return this.get().lotteryinfo
             
        },
        getNeed: function () {
            return this.get().lotteryneed;
             
        },
        getqdjh: function () {
            return this.get().libao1;
             
        },
        getqdzx: function () {
            return this.get().libao2;
             
        },
        getzouka: function (idx) {
            
            return this.get().zhouka;
        },
        getNum: function (idx) {
            
            return this.get().fightnum;
        },
        getMaxLayer: function () {
       
             return this.get().maxlayer;
        },
        getConfByzj: function (id) {
            var conf =X.clone(this.get().guanka);
            var arr = [];
        ...
        getHeroData: function (conf) {
            var arr = [];
            for(var k in conf){
                for(var i = 0 ;i <conf[k].num ; i++ ){
                    var obj = X.clone(conf[k])
                    obj.tid = obj.hid + (i+1 + "") + "123456"
                    obj.zhongzu = G.gc.hero[obj.hid].zhongzu
                    arr.push(obj)
                }
            }
            return arr;
    ...
    G.class.newyear3 = {
        get: function () {
            return G.gc.newyear3;
        },
        getlbConf: function () {
            var conf =X.clone(this.get().libao);
            var arr = [];
            for (var i in conf){
                if(conf[i].proid){
                    conf[i].idx = i;
