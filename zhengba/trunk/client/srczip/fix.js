(function(){
    ...
    cc.getRect = function (node) {
        ...
        //}
        return cc.rect(pos.x - size.width * scaleX * ap.x, pos.y - size.height * scaleY * ap.y,Math.abs(size.width * scaleX),Math.abs(size.height * scaleY));
    };
    ...
    X.bUi.prototype.showToper = function(){
        ...
        if(cc.sys.isObjectValid(G.view.toper.ui)){
            ...
            this.toper = G.view.toper;
            G.view.toper.from = this;
        }
    };
    //改变toper的货币值显示
    ...
    cc.enableScrollBar = function (scrollView,isEnable) {
        ...
        if (!cc.sys.isObjectValid(scrollView)) return;
    ...
    ccui.ListView.prototype.jumpToIdx = function (idx,args) {
        ...
        if (type == 'horizontal') {
            ...
            if (innerVal - val < visibleVal) {
                ...
                if (args.action) {
                    me.getInnerContainer().runActions([
                        cc.moveTo(0.5, -val,0)
                    ]);
                } else {
    ...
    ccui.LoadingBar.prototype.setPercentAni = function (val, callback, bool) {
    	var that = this;
