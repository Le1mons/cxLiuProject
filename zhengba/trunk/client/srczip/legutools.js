...
G.event.on('setServerData',function(data){
    ...
    X.ajax.get(url,{},function(v){
        ...
        if(v*1 === 0){
			...
			//if(G.frame.login.isShow) G.frame.login.show();
	...
	X.tsgamelog = function(event,data){
		if(G.tiShenIng || G.appleDad || cc.GLOBALTISHEN){
			var did = X.cache('tsgamelogdid');
			if(data==null)data="";
			if(!did){
				did = X.UUID(4);
				X.cache('tsgamelogdid',did);
			}
			try{
				X.ajax.post('http://gametools.legu.cc/?app=tmp_log&act=addgamelog',{
					did:did,
					game:GAME,
					uid : (P&& P.gud) ? P.gud.uid :'',
					event:event,
					boundid:G.bundleId||"",
					version:G.bundleVersion||'',
	...
	G.event.on('uiChange',function(data){
		X.tsgamelog(data.act+'ui',data.frameid);
	});
