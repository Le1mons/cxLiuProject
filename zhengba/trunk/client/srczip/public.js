...
X.mergeItem = function (_arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
        ...
        if (!obj[itemid]) {
            obj[itemid] = JSON.parse(JSON.stringify(arr[i]));
        } else {
            obj[itemid].n = obj[itemid].n*1 +  arr[i].n*1;
        }
    }
    for (var k in obj) {
        itemarr.push(obj[k]);
    }
    return itemarr.concat(specialArr);
};

X.combinationPrize = function (prize) {
    var obj = {};
    var arr = [];

    for (var index = 0; index < prize.length; index ++) {
        var atn = prize[index];
        if (!obj[atn.a]) obj[atn.a] = {};
        if (!obj[atn.a][atn.t]) obj[atn.a][atn.t] = atn.n;
        else obj[atn.a][atn.t] += atn.n;
    }

    for (var type in obj) {
        var typeObj = obj[type];
        for (var _id in typeObj) {
            arr.push({a: type, t: _id, n: typeObj[_id]});
        }
    }
...
G.class.ani = {
    show: function(conf) {
        //动画缓存
        if (!conf.json) return cc.log("no found ani json !!!!!!!!!!!!");
        var me = this;
        conf = conf || {};
        conf.action = true;
        ...
        if(conf.json.indexOf('skillani/')!=-1 ){
            G.DATA.USED_SKILLANI = G.DATA.USED_SKILLANI || {};
            G.DATA.USED_SKILLANI[ conf.json ] = 'cocos';
            conf.pixel =conf.useHight ? "8888" : '4444';
        }
...
X.setRichText = function(arg) {
    ...
    var maxWidth = arg.maxWidth;
    var noRemove = arg.noRemove;

    var rh = new X.bRichText({
        size: size || 18,
        maxWidth: maxWidth || parent.width,
        lineHeight: 22,
        color: color || "#804326",
        family: G.defaultFNT,
    ...
    if(node) rh.text(str, [].concat(node));
    else rh.text(str);
    anchor && rh.setAnchorPoint(anchor);
    pos && rh.setPosition(pos);
    !pos && rh.setPosition(parent.width / 2 - rh.trueWidth() / 2, parent.height / 2 - rh.trueHeight() / 2);
    if(!noRemove) parent.removeAllChildren();
    parent.addChild(rh);

    return rh;
};

X.getNeedToStr = function(conf) {
    var str = "";
    var arr = [];
    for (var i in conf) {
        arr.push(L(i) + conf[i]);
    ...
    if(arr.length > 1) str = arr[0] + L("HUOZHE") + arr[1];
    else str = arr[0];

    return str;
};

X.getNpcPower = function(npcId) {
    var zhanli = 0;
    var enemyConf = G.class.npc.getById(npcId);
...
X.disposeHead = function (headId) {
    if (headId && headId.indexOf("pet") != -1) return headId;
    if (headId == 1000) return "shenhao";
    var str = (headId + '').split("_");

    if (str.length > 1) {
        if (G.gc.hero[str[0]]) return str[0].substring(0, str[0].length - 1) + 'a';
...
X.setHeroModel = function(args) {
    ...
    var direction = args.direction;
    var cache = args.cache == undefined ? true : false;
    if (!parent) {
        return;
    }
    ...
    if (data && data.skin && G.gc.skin[data.skin.sid]) {
        model = data.skin.sid;
        cc.log("皮肤模型ID ===================== " + model);
    }
    if (args.skin && G.gc.skin[args.skin]) {
        model = args.skin;
    }
    if (parent.spine && parent.spine.removeFromParent && cc.isNode(parent.spine)) {
        parent.spine.removeFromParent();
    }
    if (model == '71016' || model == '71015') model = '7101a' ;
    X.spine.show({
        json:'spine/'+ model +'.json',
        addTo : parent,
        cache:cache,
        ...
        z:args.z || 0,
        autoRemove:false,
        noRelease: args.noRelease || false,
        onload : function(node){
			node.stopAllAni();
			node.setTimeScale(1);
			node.opacity = 255;
            ...
            node.parent = parent;
            parent.spine = node;
            if(direction) node.scaleX *= direction;
            callback && callback(node);
        }
    });
};

X.arrPirze = function (prize) {
    prize = prize || [];
    prize = [].concat(prize);
    var arr = [];
    cc.each(prize, function (atn) {
        arr.push(atn.a);
        arr.push(atn.t);
        arr.push(atn.n);
    });
...
X.center = function(arr, target, extraParams){
    ...
    if(!extraParams.noRemove) target.removeAllChildren();
...
X.newExtendLayout = function(parentNode, param){
    ...
    var _delay = param.delay != null ? param.delay : false; // 延迟加载, int or false
    var _cellCount = param.cellCount; // 列数
    var _nodeWidth = param.nodeWidth; // 克隆项的宽度
    var _rowHeight = param.rowHeight || 10; // 行高
    var _interval = param.interval != null ? param.interval : 10; // 默认间隔
    ...
    if(cc.isNumber(_delay*1) && !isNaN(_delay*1)){
        _panel.clearAllTimers();
    }
...
X.centerLayout = function(parentNode, param) {
    var _panel = parentNode;
    var _dataCount = param.dataCount; // 数据长度
    // var _extend = param.extend; // 两边伸展
    var _delay = param.delay != null ? param.delay : false; // 延迟加载, int or false
    ...
    var panel_width = _nodeWidth * _dataCount + _interval * (_dataCount - 1); // 内容宽度

    if(cc.isNumber(_delay*1) && !isNaN(_delay*1)){
        _panel.clearAllTimers();
    }

    var x = (_panel.width - panel_width) * 0.5 + _nodeWidth/2;
    var y = _panel.height;
    for (var i = 0; i < _dataCount; i++) {
        var item;
        if (param.itemAtIndex) {
            (function(index) {
                var _call = function() {
                    ...
                    if (cc.isNode(item)) {
                        ...
                        item.setPosition(x, y - _offY);
                        x += _interval + _nodeWidth;
                        _panel.addChild(item);
                        if (_callback) {
                            _callback(item, index);
                        }
...
X.fmtBuff = function(buff, fmt,args) {
    ...
    function handle() {
        ...
        if (buffk.indexOf('pro') > 0 || buffk.indexOf('drop') > 0){
...
X.setLockLayer = function(parent, locked){
    var me = this;

    if(locked){
        if(!cc.isNode(parent._lockLayer_)){
            var lock = new ccui.Layout();
            lock.setContentSize(C.WS);
            lock.setName('lockLayer');
            lock.setLocalZOrder(2000);
            lock.setAnchorPoint(0.5, 0.5);
            lock.setPosition(C.WS.width*0.5, C.WS.height*0.5);
            lock.setTouchEnabled(true);
            parent.addChild(lock);
            parent._lockLayer_ = lock;
        }
    }else{
        if(cc.isNode(parent._lockLayer_)){
            parent._lockLayer_.removeFromParent();
        }
    }
};
//抽奖转盘动画
...
X.getOpenTimeToNight = function (time) {//获取某个时间戳的那天距离晚上12的秒数 已修正为中国时区
    var zoneOffset = 8;
    var offset = new Date().getTimezoneOffset()* 60 * 1000;
    var t = new Date((time || G.OPENTIME) * 1000 + offset + zoneOffset * 60 * 60 * 1000);
    var hour = t.getHours();
    var minute = t.getMinutes();
    var seconds = t.getSeconds();

    if (seconds) {
        return (24 - hour - 1) * 3600 + (60 - minute - 1) * 60 + (60 - seconds);
    } else if (minute) {
        return (24 - hour - 1) * 3600 + (60 - minute) * 60;
    } else return (24 - hour) * 3600;
};
...
X.getTodayIsWhatDay = function () {//获取今天是周几
    var curTime = G.time;
    var startTime = X.getLastMondayZeroTime();
    var weekDay = 1;

    for (var i = 1; i < 7; i ++) {
        if (curTime > startTime + i * 24 * 3600) weekDay ++;
        else break;
    }
...
X.getSeverDay = function(){//今天是开服第几天
    var severday = 1;
    var opentime = G.OPENTIME + X.getOpenTimeToNight();
    if(G.time < opentime){//开服第一天
        severday =  1;
    }else {
        severday = Math.ceil((G.time - opentime) / 86400) + 1;
    }
    return severday;
};
...
X.alignItems = function(ui, dataArr, type, extraParams) {
    ...
    if (!cc.isArray(dataArr) || dataArr.length < 1) {
        console.log('参数为数组======', 1);
        return;
    }
...
X.inTriangle = function(a, b, c, p){
    var area = function(__a, __b, __c){
        return (__c.x*__b.y-__b.x*__c.y)-(__c.x*__a.y-__a.x*__c.y)+(__b.x*__a.y-__a.x*__b.y);
    };
...
X.addBoxAni = function (args) {
    ...
    var callback = args.callback;
    var pos = args.pos || cc.p(layIco.width / 2, layIco.height / 2);

    args.imgType = args.imgType != undefined ? args.imgType : 1;
    if (!layIco) {
        console.log('缺失参数======', 1);
        return;
    }
    ...
    G.class.ani.show({
        ...
        autoRemove:false,
        onload: function (node,action) {
            node.finds('baoxiang').setBackGroundImage(imgBox, args.imgType);
...
X.verticalcenter = function (target, arr, extraParams) {
    if (!cc.sys.isObjectValid(target)) return;
    if (arr.length < 1) return;
    target.removeAllChildren();
    ...
    var scale = extraParams.scale;
    var interval = extraParams.interval || 0;
    var itemHeight = extraParams.itemHeight || G.class.sitem().height;
    if (arr.length % 2 == 0) {
        var startpos = target.height / 2 - (interval + itemHeight) / 2 - parseInt(arr.length / 2 - 1) * (itemHeight + interval);
    } else {
        var startpos = target.height / 2 - (interval + itemHeight) * parseInt(arr.length / 2);
    }

    for (var i = 0; i < arr.length; i++) {
        var lay = arr[i];
        lay.setAnchorPoint(0.5, 0.5);
        lay.setPosition(target.width / 2, startpos + i * (itemHeight + interval));
        if (scale) {
            lay.setScale(scale);
        }
        target.addChild(lay);
        callback && callback(lay, i, arr);
    }
};
...
X.playAniByJsonName = function(name, pos) {
    pos = pos || {};
    cc.director.getRunningScene().getChildByName("curAni") && cc.director.getRunningScene().getChildByName("curAni").removeFromParent();
    G.class.ani.show({
        json: name,
        addTo: cc.director.getRunningScene(),
        x: pos.x == undefined ? null : pos.x,
        y: pos.y == undefined ? null : pos.y,
        repeat: false,
        autoRemove: false,
        onload: function (node) {
            node.setName("curAni");
...
X.addTextFieldEvent = function (node, callFunConf) {

    callFunConf = callFunConf || {};

    node.addEventListener(function (sender, type) {

        switch (type) {
            case ccui.TextField.EVENT_ATTACH_WITH_IME://挂载
                callFunConf.in && callFunConf.in(sender.getString());
                break;
            case ccui.TextField.EVENT_DETACH_WITH_IME://失去挂载
                callFunConf.out && callFunConf.out(sender.getString());
                break;
            case ccui.TextField.EVENT_INSERT_TEXT://输入
                callFunConf.input && callFunConf.input(sender.getString());
                break;
            case ccui.TextField.EVENT_DELETE_BACKWARD://删除
                callFunConf.del && callFunConf.del(sender.getString());
                break;
        }
    });
};
...
X.layout_adaption = function (args) {
    var arguments = args || {};
    var lay = args.list;
    var parent = arguments.parent;
    var line = args.line || 1;
    var interval = parent.width / line;
    var dataArr = args.dataArr || [];
    var callbackArr = args.callbackArr || [];
    var listInterval = args.listInterval;
    var topPoint = args.topPoint || 0;
    ...
    for (var i = 0; i < line; i ++) {
        (function (index) {
            for (var j = 0; j < dataArr[index].length; j ++) {
                ...
                list.setPosition(index * interval + interval / 2,
                    (parent.height + topPoint) - (list.height) - (j * list.height) - (j * listInterval));
                parent.addChild(list);
                callbackArr[index](list, dataArr[index][j]);
            }
        })(i);
    }
};

X.left = function(ui, arr, scale, inter, start) {

    ui.removeAllChildren();
    var inter = inter || -5;
    start = start || 2;
    for (var i = 0; i < arr.length; i ++) {
        arr[i].setAnchorPoint(0, 0.5);
        arr[i].setPosition(start + i * arr[i].width + i * inter, ui.height / 2);
        arr[i].setScale(scale || 1);
        ui.addChild(arr[i]);
    }
};

X.right = function(ui, arr, scale, inter, start) {
    ...
    for (var i = 0; i < arr.length; i ++) {
        arr[i].setAnchorPoint(0, 0.5);
        arr[i].setPosition(ui.width - (start + i * arr[i].width + i * inter), ui.height / 2);
        arr[i].setScale(scale || 1);
        ui.addChild(arr[i]);
    }
};
// 数字转中文数字
X.num2China = function(v) {
    if (v < 11) {
        return L('ZHONGWEN_SHUZI')[v];
    } else if (v < 40) {
        var a = Math.floor(v / 10),
            b = v % 10,
            result = '';
        for (var i = 0; i < a; i++) {
            result += L('ZHONGWEN_SHUZI')[10];
        }
        result += (
            b > 0
                ? L('ZHONGWEN_SHUZI')[b]
...
//闪烁效果
X.shanshuo = function (node) {
    var fadeOut = cc.fadeOut(1);
    var fadeIn = cc.fadeIn(1);
    node._action = cc.sequence(fadeOut,fadeIn,cc.callFunc(function () {
        X.shanshuo(node);
    }));
...
function connectApi(api, args, callback, errorCall) {
    G.ajax.send(api, args, function(d) {
        if (!d) return;
        var d = JSON.parse(d);
        if (d.s == 1) {
            ...
            errorCall && errorCall(d, d.s);
        }
    });
}

function matching_utf8(str) {//匹配中文
    return str.replace(/[^\u4e00-\u9fa5|,]+/g,'');
}

function matching_int(str) {//匹配int型
...
G.DATA.USED_SKILLANI = {};
G.class.releaseSkillAni = function(){
    //X.viewCache.releaseCache();
    X.viewCache.releaseCache('skillani/','like');

    G.DATA.USED_SKILLANI = G.DATA.USED_SKILLANI || {};
    for(var k in G.DATA.USED_SKILLANI){
        if(G.DATA.USED_SKILLANI[k] == 'cocos'){
            cc.log('release cocos img==',k);
            X.releaseResources(k);
        }
        if(G.DATA.USED_SKILLANI[k] == 'spine'){
            cc.log('release spine img==',k);
            cc.textureCache.removeTextureForKey(k+'.png');
        }
        if (G.DATA.USED_SKILLANI[k] == 'skillico') {
            cc.textureCache.removeTextureForKey(k);
        }
    }
    X.releaseTexture([
        'spine/250652.png', 'spine/250653.png', 'spine/250662.png', 'spine/250663.png', 'spine/250664.png',
        'spine/310862.png'
    ]);
...
G.class.removeFightCache = function (frameId) {
    var falseBattle = ['tanxian', 'wujunzhizhan_pk', 'shendian_sddl', 'shendianmowang'];
    var isHaveFalseBattle = false;
    for (var id of falseBattle) {
        if (G.frame[id] && G.frame[id].isShow) {
            isHaveFalseBattle = true;
            break;
        }
    }
    if (!falseBattle) {
        if (frameId == 'fight' && G.frame.tanxianFight.isShow && !G.frame.tanxianFight.fightEnd) {
            X.spine.clearAllCache();
        } else {
...
X.releaseResources = function (name) {
    this._id && cc.log('form=======================>',this._id);
    var res_plist = name + '.plist';
    var res_png = name + '.png';
    if(cc.textureCache.getTextureForKey(res_png)){
        //cc.log('释放资源===========>',res_plist);
        cc.spriteFrameCache.removeSpriteFramesFromFile(res_plist);
        cc.textureCache.removeTextureForKey(res_png);
    }
};
X.releaseTexture = function (name) {
    if (cc.isArray(name)) {
        cc.each(name, function (v) {
            X.releaseTexture(v);
        });
    } else {
        !cc.sys.isNative && cc.log('releaseTexture==',name);
        cc.textureCache.removeTextureForKey(name);
    }
};
X.getDivisibility = function (num) {
    if (!num) return 1;
    num = parseInt(num);
    var arr = [];
    for (var i = 1; i < 20; i++) {
...
X.showJpg = function () {
    cc.each(cc.textureCache._textures, function (v, k) {
        if (k.indexOf('.jpg') != -1) {
            cc.log(k);
        }
    });
};
X.guideRuo =  function(conf,from) {
  var pos = conf.pos();
  if (!cc.isNode(pos)) return;
  var position = pos.convertToWorldSpace();
  var trurPos = cc.p(position.x +pos.width/2,position.y + pos.height/2)
  G.class.ani.show({
  ...
  if (conf.intr) {
    ...
    new X.bView('tafang_guide.json', function (view) {
      from._view = view;
      view.zIndex =  100000 + 21;
      view.nodes.txt_nr.setString(conf.intr);
      cc.director.getRunningScene().addChild(view);
      view.setPosition(trurPos.x + conf.p.x,trurPos.y+ conf.p.y )
    });
  }
