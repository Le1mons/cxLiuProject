...
(function () {
    ...
    var fun = X.bUi.extend({
        ...
        checkRedPoint: function(){
            ...
            if (G.hongdian.checkTqlb()) {
                G.setNewIcoImg(me.nodes.btn_giftbag);
                me.nodes.btn_giftbag.getChildByName("redPoint").setPosition(110, 60);
            } else {
                G.removeNewIco(me.nodes.btn_giftbag);
            }

            //激活月卡特权每日奖励且未领取奖励
            if(G.frame.chongzhi.DATA.yktq != undefined && G.frame.chongzhi.DATA.yktq == false){
                G.setNewIcoImg(me.nodes.btn_topup);
                me.nodes.btn_topup.getChildByName("redPoint").setPosition(110, 60);
            }else {
                G.removeNewIco(me.nodes.btn_topup);
            }
        },
        onHide: function () {
            var me = this;
            ...
            if (G.frame.yingxiong.isShow) {
                if (G.frame.yingxiong._panels && G.frame.yingxiong._panels.getHeroCell){
                    G.frame.yingxiong._panels.getHeroCell(function () {
                        G.frame.yingxiong.setHeroCell();
                    });
                }
            }
        },
