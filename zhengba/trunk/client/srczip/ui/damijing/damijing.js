    ...
    var fun = X.bUi.extend({
        ...
        bindBtn: function () {
            ...
            me.ui.finds("btn_an").click(function (sender) {
                if(me.fightAni) return;
                if(me.isAllDead) {
                    G.tip_NB.show(L("ALL_HERO_DEAD"));
                    return;
                }
                var time = G.time;
                var layer = G.frame.damijing.DATA.layer;
                    ...
                    if(data.s == 1) {
                        ...
                        if(X.cacheByUid("dmj_jumpFight")){
                            if(data.d.fightres.winside) {
                                G.frame.fight.DATA = data.d.fightres;
                            ...
                            G.frame.fight.once('show', function () {
                                G.frame.fight.pve_start({
                                    stage_id: layer,
                                    stage_name: G.gc.watcher[layer].npc,
                                    pve_id: 'shouwangzhemijing'
                                });
            ...
            G.class.ani.show({
                ...
                onend: function () {
                    me.playAni(function () {
                        ...
                        me.ui.finds("ico_qizi").show();
                        me.ui.finds("btn_an").show();
                        G.class.hero.getSoundByHid(me.soundId);
                    });
                }
            });
        ...
        setLevelInfo: function() {
            ...
            var cengshu = parseInt((level - 1) / 100);
            if (cengshu>9)cengshu=9;
            var str = X.STR(G.class.watchercom.getTitle(cengshu), level);
            me.lv = level;
                        ...
                        me.ajax("watcher_useitem", [node.idx * 1 - 1, node.idx], function (str, data) {
                            if(data.s == 1) {
                                G.event.emit('sdkevent',{
                                    event:'mj_useItem',
                                    data:{
                                        consume:[{a:conf[node.idx].name,t:conf[node.idx].name,n:1}],
                                    }
                                });
        ...
        addEnemy: function() {
            ...
            if (me.DATA.npc.herolist[0].zhongzu * 1 == 7){
                me.nodes.panel_zz2.setBackGroundImage('img/public/ico/ico_zz11_s.png', 1);
            } else {
            ...
            me.soundId = me.DATA.npc.headdata.head;
            if(!me.first) G.class.hero.getSoundByHid(me.soundId);
            me.first = true;
        },
