...
(function () {
    ...
    G.event.on("addfriend", function () {
        ...
        G.hongdian.getData("friend", 1, function () {
            if(G.frame.friend.isShow) {
                G.frame.friend.isShow.checkRedPoint();
            }
        });
    });
    ...
    var fun = X.bUi.extend({
        ...
        checkRedPoint: function(){
            var me = this;
            var arr = ["yinji", 'apply'];
            var btns = [me.nodes.listview.children[0], me.nodes.listview.children[2]];
            for(var i = 0; i < arr.length; i ++){
                if(G.DATA.hongdian.friend[arr[i]] > 0){
        ...
        getData: function(callback, errCall, isOnlyUid) {
            var me = this;
            G.ajax.send('friend_open', [isOnlyUid], function(d) {
                if (!d) return;
                var d = JSON.parse(d);
                if (d.s == 1) {
                    me.DATA = d.d;
                    G.DATA.friend = d.d.friend;
                    callback && callback();
                } else {
                    G.DATA.friend = [];
                    errCall && errCall();
