        ...
        setItem: function(ui, data) {
            ...
            head.touch(function(sender, type) {
                if (type == ccui.Widget.TOUCH_ENDED) {
                    if(P.gud.sid != sender.data.sid) {
                        return G.tip_NB.show(L("LZWY"));
                    }
            ...
            var isKf = P.gud.sid == d.uid.split("_")[0] ? "" : L("KF");
            ui.nodes.txt_player_name1.setString(d.name + isKf);
            ui.nodes.btn_remove1.uid = d.uid;
            ui.nodes.btn_remove1.sid = d.uid.split("_")[0];
            ui.nodes.btn_remove1.show();
            ui.nodes.btn_remove1.click(function(sender) {
                if(sender.sid && sender.sid != P.gud.sid && G.DATA.kfpb[sender.uid]) {
                    G.DATA.kfpb[sender.uid] = undefined;
                    delete G.DATA.kfpb[sender.uid];
        ...
        getBlackList: function(callback) {
            ...
            G.ajax.send('friend_blacklist', [], function(d) {
                ...
                if (d.s == 1) {
                    ...
                    var kfpb = G.DATA.kfpb;

                    for (var i in kfpb) {
                        var isHave = false;

                        for (var j in me.blacklist.blacklist)  {
                            if(me.blacklist.blacklist[j].uid && me.blacklist.blacklist[j].uid == i) {
                                isHave = true;
                                break;
                            }
                        }

                        if(!isHave) me.blacklist.blacklist.push(kfpb[i]);
                    }
