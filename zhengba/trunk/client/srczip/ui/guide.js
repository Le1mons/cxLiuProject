    ...
    G.class.guideFun = {
        "closeAllFrame": function () {
            ...
            cc.callLater(function () {
                G.guidevent.emit("closeAllFrameOver");
            });
        },
        ...
        "scrollToFst": function (conf) {
            G.frame.julongshendian.hua(conf.stype);
        },
    	...
    	"pauseFight" : function(conf){
    		//暂停战斗
    		if(G.frame.tanxianFight.DATA)G.frame.tanxianFight.DATA.isPause = true;
    		cc.callLater(function(){
    	...
    	"resumeFight" : function(){
    		//继续战斗
    		if(G.frame.tanxianFight.DATA)G.frame.tanxianFight.DATA.isPause = false;
    		G.frame.tanxianFight.tranRound && G.frame.tanxianFight.tranRound();
    	...
    	"closeFightWithWin" : function(){
    		...
    		G.frame.fight_tanxian_win.remove();
...
G.guide.view.setTimeout(function(){
    G.guide.view.lockClick = function(sender,type){
       G.frame.yingxiong_fight.top.changeItem(
          G.frame.yingxiong_fight.ui.finds('panel_qp').finds('0'),
          G.frame.yingxiong_fight.ui.finds('panel_hp').finds('2')
          );
       cc.isNode(aniNode) && aniNode.removeFromParent();
        ...
        "eventprize": function (conf) {
            G.view.mainView.getEventPrize(2, G.gc.event.event[2].option[0]);
            if (!conf.afterguide) G.guide.remove();
        }
};
...
G.view.GuideView = X.bView.extend({
    ...
    onOpen: function () {
        ...
        onShow: function () {
            var me = this;
            // me.ui.finds('skip').opacity = 0;
            me.ui.finds('skip').hide();
            me.nodes.panel_dgdh.hide();
            me.nodes.panel_dgdh.setTouchEnabled(true);
            G.class.ani.show({
                json: "ani_tiaoguo",
                addTo: me.nodes.panel_dgdh,
                repeat: true,
                autoRemove: false
            });
        ...
        _hideAllType : function(){
        	...
        	me.lockClick = function(){
        	    if (G.frame.tanxianFight.isShow || G.frame.fight.isShow) {
        	        return G.tip_NB.show(L("guide_tip"));
                }
        	    cc.log('lockClick...');
        	};
        ...
        parseSingleGuideConf : function(conf){
        	...
        	var number = conf.afterguide.split("_")[1];
        	if (X.inArray(G.guide.eventGroup, number)) {
        	...
        	if(conf.type=='1'){
        		...
        		if(me.nodes.duibai.finds('renwu').getChildrenCount() == 0 ){
        			//加动画
                    var model = conf.hid || "xinshourenwu";
        			X.spine.show({
                ...
                // if(conf.skip)me.ui.finds('skip').show(); //需求变更为任何环境下10秒后显示跳过按钮
...
_parseNeedCondition : function(conf){
      ...
      _setFingerInfo : function(conf){
          ...
          me.__skipTimer = me.setTimeout(function(){
              me.nodes.panel_dgdh.show();
              me.nodes.panel_dgdh.click(function () {
                  X.delCacheByUid('guideStepInfo');
                  X.cacheByUid('jumpGuide',1);
                  G.guide.remove();
              });
          },10000);
           ...
           if(node){
    				...
    				if(!cc.sys.isNative || cc.rectContainsPoint(_rect, sender.getTouchEndPosition())){
                       ...
                       cc.log('不在区域内，todo提示效果....',sender.getTouchEndPosition());
                        ...
                        if(X.inArray(["btn_qr2$", "txt_djgb$", "btn_fh$", "btn_guanbi$"], node.getName())) {
                            ...
                            if(!me.focusIn) {//聚焦效果
                                me.focusIn = true;
                                me.nodes.img_jujiao.setPosition(pos);
                                me.nodes.img_jujiao.setScale(15);
                                me.nodes.img_jujiao.show();
                                me.nodes.img_jujiao.runActions([
                                    cc.scaleTo(0.5, 1),
                                    cc.fadeTo(1, 100),
                                    cc.callFunc(function () {
                                        me.focusIn = false;
                                        me.nodes.img_jujiao.hide();
                                    })
                                ]);
                            }
						}
                ...
                if(X.cacheByUid("guideFocusPos")) {
                    var lastPos = X.cacheByUid("guideFocusPos");
                    me.fingerAni.setPosition(lastPos);
                    G.DATA.noClick = true;
                    me.fingerAni.runActions([
                        cc.moveTo(0.3, pos),
                        cc.callFunc(function () {
                            G.DATA.noClick = false;
                            me.fingerAction.play("wait", true);
                        })
                    ]);
                    X.cacheByUid("guideFocusPos", pos);
                } else {
...
G.guide = {
   view : null,
    ...
    eventGroup: [10, 20, 60, 160, 180, 220, 330, 360, 410, 450, 510, 570, 660, 700, 750, 870, 930, 980],
...
//	}
   ...
   checkGuide();
     X.releaseTexture(['img/bg/bg_loading1.jpg', 'img/bg/bg_denglu.jpg']);
});
        ...
        cc.director.getRunningScene().setTimeout(function(){
        	eachStep(function(emitCond,gpid,stepid){
        		...
        		if( emitCondArr[0] == 'lv' && d.olv < emitCondArr[1]*1 && d.lv >= emitCondArr[1]*1 ){
        		    if (G.gc.open[emitCondArr[1]] && G.gc.open[emitCondArr[1]].time && G.OPENTIME > G.gc.open[emitCondArr[1]].time) return null;
        			G.frame.playerlvup.once('close',function(){
        				...
        				if(G.frame.wangzhediaosu.isShow) G.frame.wangzhediaosu.remove();
