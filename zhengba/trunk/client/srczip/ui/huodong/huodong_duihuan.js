...
(function () {
    ...
    G.class.huodong_duihuan = X.bView.extend({
        ...
        setItem:function (ui,data,pos) {
            ...
            //消耗图标
            me.setIcon(ui, data.need ,true);
            ui.nodes.btn.needKey = data.need[0].t;
            ...
            ui.nodes.btn.click(function(sender, type){
                ...
                if (G.class.getOwnNum(sender.data.need[0].t, sender.data.need[0].a) < sender.data.need[0].n) {
                    return G.tip_NB.show(G.class.getItem(sender.data.need[0].t, sender.data.need[0].a).name + L("BUZU"));
                }

                function buy(num) {
                    me.ajax('huodong_use',[me._data.hdid,num,sender.pos],function(str,d) {
                        if(d.s == 1) {
                            var prize = JSON.parse(JSON.stringify(sender.data.p));
                            for (var i = 0; i < prize.length; i ++) {
                                prize[i].n *= num;
                            }
                            var need = JSON.parse(JSON.stringify(sender.data.need));
                            for(var k = 0; k < need.length; k++){
                                need[k].n = need[k].n * num;
                            }
                ...
                G.frame.buying.data({
                    ...
                    maxNum: sender.data.buynum,
                    callback: function (num) {
                        if (sender.needKey == "rmbmoney") {
                            if (X.cacheByUid("rmbbuy")) {
                                buy(num);
                            } else {
                                G.frame.hint.data({
                                    callback: function () {
