    ...
    G.class.huodong_monthPrize = X.bView.extend({
        ...
        getData: function(callback){
            ...
            G.ajax.send("weekmonthlibao_open", [me.type], function (d) {
                ...
                if (d.s == 1) {
                    me.DATA = d.d;
                    if(!me.DATA.itemdict[me.type + "_" + 0]) {
                        me.DATA.itemdict[me.type + "_" + 0] = {};
                        me.DATA.itemdict[me.type + "_" + 0].num = 1;
                    }
                    callback && callback();
                }
            }, true);
        ...
        setTable: function () {
            ...
            for(var i = 0; i < keys.length; i ++){
                ...
                data[keys[i]].conf = conf[me.DATA.key].itemdict[keys[i]];
        ...
        setItem: function (ui, data) {
            ...
            X.render({
                btn: function (node) {
                    ...
                    if(data.conf.rmbmoney == 0 && data.num > 0){
                        G.setNewIcoImg(node);
                    ...
                    node.click(function (sender, type) {
                        if(sender.data.conf.rmbmoney == 0){
                            me.ajax('weekmonthlibao_receive',[me.type,sender.data.pid],function (str,data) {
                                if(data.s == 1){
                                    G.frame.jiangli.data({
                                        prize:sender.data.conf.p
                                    }).show();
