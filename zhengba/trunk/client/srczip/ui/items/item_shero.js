...
// 英雄信息界面的英雄图标
G.class.shero = function (data,showName, item, isHelp) {
    item = item || new G.class.sheroTemp();
    item.panel_tx.loadTextureNormal("img/public/img_renwu.png", 1);
    if (!data) return item;
    ...
    var tx = (data.star || conf.star) > 9 ? (conf.tenstarico ? conf.tenstarico : conf.heroico) : conf.heroico;
    if(data.fakemodel) tx = data.fakemodel;

    var headImg = G.class.fmtItemICON(tx);
    if (data.skin && data.skin.sid) headImg = data.skin.sid;
    item.panel_tx.loadTextureNormal('ico/itemico/' + headImg + '.png', 0);
    
    //名称
    item.titleNode(showName);
    if (showName) {
        setTextWithColor(item.title, conf.name, G.gc.COLOR[conf.color]);
    }
    //等级
    item.lvNode(data.lv);
    if (data.lv) {
    ...
    if (data.tid && X.cacheByUid("hero_remarks" + data.tid)) {//英雄备注
        item.bzNode(X.cacheByUid("hero_remarks" + data.tid));
    }

    if (isHelp) {
        var helpImg = new ccui.ImageView("img/public/ico_zmbz.png", 1);
        helpImg.setAnchorPoint(0.5, 0.5);
        helpImg.setPosition(item.width - helpImg.width / 2, 15);
        item.addChild(helpImg);
        helpImg.zIndex = 99;
    }
...
G.class.shero_extneed = function (data, armyinfo, showName, txt) {
    var n;
    var color_n;
    ...
    if(data.sxhero){
        ...
        n = true;
        color_n = (data.samezhongzu || data.jichuzhongzu) ? -1 : 0;
    }

    var item = new G.class.sheroTemp();
	item.numNode(true);

	if (showName) {
	    if (data.t) {
	        setTextWithColor(txt, G.gc.hero[data.t].name, "#ffffff");
        }
	    if (data.star) {
	        if (data.jichuzhongzu) {
                setTextWithColor(txt, data.star + L("star") + '四' + L("XI"), "#ffffff");
            } else {
                setTextWithColor(txt, data.star + L("star") + L("zhongzu_" + G.gc.hero[armyinfo.hid].zhongzu), "#ffffff");
            }
    ...
    }else if (data.jichuzhongzu) {
        heroico = 'hero_' + (data.star >= 6?6:data.star);
        zhongzu = 9;
        dengjielv = data.star;
        ownNum = G.frame.yingxiong.getHeroNumByZzAndStar([1, 2, 3, 4],dengjielv);
    }else{
        if( data.t){
    ...
    item.setEnabled = function (v) {
        ...
        if(v){
            ...
            zhongzu && item.panel_zz.setBackGroundImage('img/public/ico/ico_zz' + (zhongzu + 1) + '_s.png', 1);
        }else{
...
G.class.sheroTemp = ccui.Layout.extend({
    ...
    bzNode: function (str) {
        if(!cc.isNode(this.panel_bz)){
            this.panel_bz = new ccui.Layout();
            this.panel_bz.setName('panel_zz');
            this.panel_bz.setAnchorPoint(0.5, 0.5);
            this.panel_bz.setContentSize(32, 32);
            this.panel_bz.setBackGroundImage("img/public/img_wzd.png", 1);
            this.addChild(this.panel_bz);
            this.panel_bz.zIndex = 99999;

            var txt = new ccui.Text(str, G.defaultFNT, 16);
            txt.setAnchorPoint(0.5, 0.5);
            X.enableOutline(txt, "#523018", 2);
            txt.setPosition(this.panel_bz.width / 2, this.panel_bz.height / 2);
            this.panel_bz.addChild(txt);
        }
        this.panel_bz.setPosition(this.panel_bz.width / 2, this.panel_bz.height / 2);
    },
    ...
    setEnabled : function (v) {
        ...
        if(v){
            ...
            if (this.conf.zhongzu==7){
                this.panel_zz.setBackGroundImage('img/public/ico/ico_zz11_s.png', 1);
            } else {
                this.panel_zz.setBackGroundImage('img/public/ico/ico_zz' + ((this.conf.zhongzu * 1) + 1) + '_s.png', 1);
            }

            if (this.conf && this.conf.color) {
                this.setBackGroundImage('img/public/ico/ico_bg'+ this.conf.color +'.png', 1);
            }
        }else{
            ...
            if (this.conf.zhongzu==7){
                this.panel_zz.setBackGroundImage('img/public/ico/ico_zz11_sh.png', 1);
            } else {
    ...
    //对勾设置
    setGou : function (show, imgPath) {
	    imgPath = imgPath || "img_gou";
    	if(!show){
			cc.isNode(this.gou) && this.gou.removeFromParent();
			delete this.gou;
		}else{
			if(!cc.isNode(this.gou)){
				this.gou = new ccui.ImageView('img/public/' + imgPath + '.png',1);
    ...
    //锁设置
    setSuo : function (show, imgPath) {
        imgPath = imgPath || "img_mc";
        if(!show){
            cc.isNode(this.suo) && this.suo.removeFromParent();
            delete this.suo;
        }else{
            if(!cc.isNode(this.suo)){
                this.suo = new ccui.ImageView('img/public/' + imgPath + '.png',1);
                this.suo.setName('suo');
                this.suo.setAnchorPoint(cc.p(0.5,0.5));
                if (!cc.isNode(this)) return;
                this.suo.setPosition(cc.p(this.width / 2,this.height / 2));
                this.suo.setLocalZOrder(1000);
                this.addChild(this.suo);
                //var imgGou = this.gou;
            }else{
                this.suo.show();
            }
    ...
    //加号
    setAdd : function (show) {
        if(!show){
            cc.isNode(this.jia) && this.gou.hide();
        }else{
            if(!cc.isNode(this.jia)){
                this.jia = new ccui.ImageView('img/public/img_pub_jia.png',1);
                this.jia.setName('jia');
                this.jia.setAnchorPoint(cc.p(0.5,0.5));
                this.jia.setPosition(cc.p(this.width / 2,this.height / 2));
                this.jia.setLocalZOrder(1000);
                this.addChild(this.jia);
            }else{
                this.jia.show();
            }
        }
    ...
    setHP : function(v,show,str){
    	if(!show){
			...
			if(str) {
			    var txt = new ccui.Text(str, G.defaultFNT, 12);
			    txt.setPosition(50, 3);
			    X.enableOutline(txt, "#7b531a", 1);
			    this.hp.addChild(txt);
            }
			var hp = this.hp;
            hp.setPercent(v);
    ...
    setNQ : function(v, show) {
	    if(!show) {
            ...
            if(!cc.isNode(this.nqbg)){
                ...
                this.nqbg.setBackGroundImage("img/public/jdt/img_sp_jdt_bg1.png", 1);
                this.nqbg.setPosition(0, -31);
            }
    ...
    setArtifact: function(id, isBuffAni) {
        ...
        if(G.DATA.artifact && G.DATA.artifact[id] && isBuffAni && !G.DATA.isPiao) {
            ...
            if(G.frame.yingxiong_fight.isShow){
                G.frame.yingxiong_fight.ui.addChild(bg);
            }else if (G.frame.jingjichang_gjfight.isShow){
             G.frame.jingjichang_gjfight.ui.addChild(bg);
            }
