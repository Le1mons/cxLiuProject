(function () {
    ...
    G.class.mainMenu = X.bView.extend({
        hierarchy: {
            tanxian: 5,
            tanxianFight: 10,
            fight_tanxian_fail: 13,
            fight_tanxian_win: 13
        },
        lower: {
            tanxian: 5,
            tanxianFight: 4,
            fight_tanxian_fail: 1,
            fight_tanxian_win: 1
        },
        ctor: function () {
            var me = this;
        ...
        bindBTN: function () {
            ...
            function isHave() {
                ...
                delete redPointData.watcher;
                delete redPointData.guajitime;
                delete redPointData.destiny;
                delete redPointData.artifact;
                if (redPointData.fashita.maze && redPointData.fashita.maze > 2) delete redPointData.fashita.maze;
            ...
            //公会
            if(G.tiShenIng) me.nodes.btn_gonghui.hide();
        ...
        lowerIndex: function() {
            ...
            if(G.frame.tanxianFight.isShow) {
                var minHierarchy = G.view.mainView.zIndex;

                for (var i in me.lower) {
                    if(G.openingFrame[i] != undefined) {
                        G.openingFrame[i] = minHierarchy - me.lower[i];
                        G.frame[i].ui.zIndex = minHierarchy - me.lower[i];
                        G.frame[i].visibleWithOutEmit(false);
                    }
                }
        ...
        addIndex: function() {
            ...
            if(G.frame.tanxianFight.isShow) {
                X.uiMana.closeAllFrame(true);
                var maxHierarchy = X.uiMana.getOpenFrameOrderByZindex()[0] ? X.uiMana.getOpenFrameOrderByZindex()[0].zIndex : 0;

                for (var i in me.hierarchy) {
                    if(G.openingFrame[i] != undefined) {
                        G.openingFrame[i] = maxHierarchy + me.hierarchy[i];
                        G.frame[i].ui.zIndex = maxHierarchy + me.hierarchy[i];
                        G.frame[i].visibleWithOutEmit(true);
                    }
                }

                //这里不会触发uiChange逻辑 手动调整层级
                G.view.mainMenu.zIndex = G.frame.tanxianFight.ui.zIndex + 1;
            } else {
        ...
        eventPush:function(){
            var me = this;
            var time = false;
            if(G.time >= 1588435200 && G.time <= 1588694400){
                time = true;
            }
            if(time && G.owner && (G.owner === 'dev' || G.owner ===  "changwanbl" || G.owner === "cwbl" || G.owner === "cwrybl" || G.owner === "cwblios")){
                G.frame.event_znq.show();
            }
        },
        onRemove: function () {
            var me = this;
