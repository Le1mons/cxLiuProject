...
(function () {
    //大秘境-种族-选择
    G.class.td_xuanze = X.bView.extend({
        extConf:{
            fight:{
                data: function (type) {
        ...
        setItem: function (ui, data) {
            ...
            ui.touch(function (sender, type) {
                if (type == ccui.Widget.TOUCH_NOMOVE){
                    ...
                    // }
                    G.frame.td_fightRead.posSelect = G.frame.td_fightRead.ui.convertToNodeSpace(sender.getParent().convertToWorldSpace(sender.getPosition()));
                    if (X.inArray(me.selectedData, sender.data)) {
                        G.frame.td_fightRead.posSelect.x += sender.width / 2;
                        sender.finds('widget').setGou(false);
                        me.selectedData.splice(X.arrayFind(me.selectedData,sender.data),1);
                        G.frame.td_fightRead.bottom.removeItem(sender.data);
                    } else {
                        if (me.selectedData.length >= 3) {
                            return G.tip_NB.show(L('YX_FIGHT_XZ_FULL'));
                            // for (var i = 0; i < G.frame.td_fightRead.bottom.panel_yx.children.length; i ++) {
                            //     var chr = G.frame.td_fightRead.bottom.panel_yx.children[i];
                            //     chr.children[2].children[0].setSelected();
                            // }
                        }
                        me.selectedData.push(sender.data);
                        sender.finds('widget').setGou(true);
                        G.frame.td_fightRead.bottom.addItem(sender.data);

                        if (cc.isNode(G.frame.td_fightRead.item)) {
                            G.frame.td_fightRead.item.stopAllActions();
                            G.frame.td_fightRead.item.removeFromParent();
                        }
                        var itemClone = G.frame.td_fightRead.item = sender.clone();
                        itemClone.finds('gou').hide();
                        itemClone.setPosition(G.frame.td_fightRead.posSelect);
