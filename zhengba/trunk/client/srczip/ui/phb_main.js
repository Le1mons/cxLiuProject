    ...
    var fun = X.bUi.extend({
        ...
        onShow: function () {
            ...
            new X.bView("zhuui_paihangbang.json", function (node) {
                ...
                me.view.nodes.btn_yjmb.click(function(){
                    G.ajax.send("rank_allworship", [], function (data) {
                        if (!data) return;
                        var data = JSON.parse(data);
                        if (data.s == 1) {
                            for(var k in me.idx){
                                if(!X.inArray(me.worship,me.idx[k])){
                                    me.worship.push(me.idx[k]);
                                }
                            }
                            G.frame.jiangli.data({
                                prize: data.d.prize
        ...
        setBtnState: function() {
            ...
            if(me.worship.length < X.keysOfObject(me.idx).length){
                me.view.nodes.btn_yjmb.setBright(true);
                me.view.nodes.btn_yjmb.setTouchEnabled(true);
                me.view.nodes.txt_yjmb.setTextColor(cc.color("#813D00"));
            }else{
                me.view.nodes.btn_yjmb.setBright(false);
                me.view.nodes.btn_yjmb.setTouchEnabled(false);
                me.view.nodes.txt_yjmb.setTextColor(cc.color("#6c6c6c"));
            }
