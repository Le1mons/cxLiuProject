...
(function () {
    ...
    G.class.back_dlfl = X.bView.extend({
        ...
        setBanner:function () {
            ...
            X.render({
                panel_banner: function (node) {
                    node.setBackGroundImage('img/event/img_event_banner23.png', 0);
                },
        ...
        setList:function () {
            ...
            for(var i in me.conf[me.type]){
                listdata.push(me.conf[me.type][i]);
            }
        ...
        setItem:function (ui,data,index) {
            ...
            if(me.DATA.login < index + 1){//不满足条件，不可领取
                ...
                if(X.inArray(me.DATA.receive.login,me.keyarr[index])){//已领取
            ...
            X.render({
                btn: function (node) {
                    node.click(function (sender, type) {
                        me.ajax('return_receive', [me.type, me.keyarr[index]], function (str, dd) {
                            if(dd.s == 1){
                ...
                txt: function (node) {
                    ...
                    if(me.DATA.login > index+1){
                        var txt = new ccui.Text(X.STR(L("backtip3"),index+1,index+1,index+1),G.defaultFNT, 22);
                    }else {
                        var txt = new ccui.Text(X.STR(L("backtip3"),index+1,me.DATA.login,index+1),G.defaultFNT, 22);
                    }
                    txt.setTextColor(cc.color(me.DATA.login >= index+1 ? G.gc.COLOR[1] : G.gc.COLOR[5]));
