...
(function () {
    ...
    G.class.back_mrlc = X.bView.extend({
        ...
        setBanner:function () {
            ...
            X.render({
                panel_banner: function (node) {
                    node.setBackGroundImage('img/event/img_event_banner24.png', 0);
                },
                panel_title: function (node) {
                    ...
                    rh.text(me.conf.intr[me.type]);//描述
        ...
        setItem:function (ui,data,index) {
            ...
            if(me.DATA.daily < data.pval){//不满足条件，不可领取
                ...
                if(X.inArray(me.DATA.receive.daily,index)){//已领取
            ...
            X.render({
                ...
                txt: function (node) {
                    ...
                    var txt = new ccui.Text(X.STR(L("backtip1"),data.pval,me.DATA.daily,data.pval),G.defaultFNT, 22);
                    txt.setTextColor(cc.color(me.DATA.daily >= data.pval ? G.gc.COLOR[1] : G.gc.COLOR[5]));
