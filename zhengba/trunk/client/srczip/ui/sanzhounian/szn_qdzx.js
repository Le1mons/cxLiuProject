(function () {
    ...
    var fun = X.bUi.extend({
        ...
        onOpen: function () {
            var me = this;
            me.DATA = G.DATA.szn;
            me.conf = X.clone(G.class.szn.getqdzx());
            me.setContents()
        },
            ...
            X.render({
                libao_cs: function (node) {
                    var rh = X.setRichText({
                        str: X.STR(L("szn_11"), data.buynum - (me.DATA.libao2[data.proid] || 0)),
                        parent: node,
                        color: '#be5e30',
                        size: 20
                    });
                ...
                btn_lq: function (node) {
                    node.show();
                    node.setBright(data.buynum > (me.DATA.libao2[data.proid] || 0))
                    node.setTouchEnabled(data.buynum > (me.DATA.libao2[data.proid] || 0))
        ...
        onShow: function () {
            var me = this;
            me.nodes.txt_title.setString(L('szn_12'));
            me.ui.finds("txt_rw").setString(L("szn_13"));
            me.bindBtn()
        },
        onAniShow: function () {
            var me = this;
