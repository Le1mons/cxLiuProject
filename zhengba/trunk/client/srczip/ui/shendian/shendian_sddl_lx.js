...
(function () {
    ...
    var fun = X.bUi.extend({
        ...
        getData: function(callback){
            var me = this;
            G.ajax.send("dungeon_recording", [me.data().path, me.data().layer], function (d) {
                if (!d) return;
                var d = JSON.parse(d);
                if(d.s == 1){
                    me.DATA = d.d;
        ...
        setList: function (list, conf, idx) {
            ...
            X.autoInitUI(list);
            if(conf){
                var data = conf.fightres.headdata[0];
                list.nodes.btn_lx.click(function (sender, type) {
                    conf.fightres.pvType = "video";//增加跳过类型
                    G.frame.fight.demo(conf.fightres);
                });
