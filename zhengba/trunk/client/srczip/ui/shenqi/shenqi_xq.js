    ...
    var fun = X.bUi.extend({
        ...
        setContents: function () {
            ...
            if(me.data().jh){
                ...
                var wakeRand = G.gc.shenqicom.wakerank[data.rank || 0];
                ...
                if(!data.jxlv) {
                    ...
                    var addPro = G.class.shenqi.getSqJxDps(id, data.jxlv);
                    var val = "(+" + parseInt(parseInt(interArr[1]) * ((addPro + wakeRand.shenqidpspro) / 1000)) + ")";
                    interArr[1] += val;
                    for (var i in interArr) {
                        inter += interArr[i];
                    }
                ...
                if(data.rank > 0) {
                    var str2 = "";
                    var buffArr = [];
                    var jxbuff = G.gc.shenqicom.base.wake[conf.waketype][data.jxlv];
                    str2 += L('SHENQI_JXJC');

                    for (var i in jxbuff.buff) {
                        if(i.indexOf("pro") == -1) {
                            buffArr.push(jxbuff.buff[i] + wakeRand.buff[i]);
                        } else {
                            buffArr.push((jxbuff.buff[i] + wakeRand.buff[i]) / 10 + "%");
                        }
                    }
                    buffArr.push((jxbuff.shenqidpspro + wakeRand.shenqidpspro) / 10 + "%");

                    for (var i in conf.keytitle) {
                        str2 += "<br>" + conf.keytitle[i] + " +" + buffArr[i];
                    }

                    var jxrh = X.setRichText({
                        str: str2,
                        parent: listview,
                        anchor: {x: 0, y: 1},
                        pos: {x: 0, y: 0},
