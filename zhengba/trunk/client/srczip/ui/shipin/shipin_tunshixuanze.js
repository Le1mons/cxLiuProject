    ...
    var fun = X.bUi.extend({
        ...
        setTop: function (spid) {
            ...
            if (spid && G.gc.shipincom.awake.skillList[spid.split("_")[1]]) {
                str += G.gc.shipincom.awake.skillList[spid.split("_")[1]].intr;
            }
            rh.text(str);
            var offsetY = rh.trueHeight();
            if (offsetY > layBuff.height) {
                panel.nodes.panel_bg.height += offsetY - layBuff.height + 10;
            ...
            var state = {
                btns:[
                    ...
                    },
                    {
                        setBtn: function (item) {
                            //加入十个
                            item.setTitleText(L('BTN_MAX'));
                            item.setTitleColor(cc.color(G.gc.COLOR.n13));
                            item.touch(function (sender, type) {
                                if (type == ccui.Widget.TOUCH_ENDED) {

                                    var num = 0;
                                    var exp = G.frame.shipin_shengji.panelBottom.curExp || 0;
                                    var needExp = G.frame.shipin_shengji.panelBottom.needExp;
                                    var oneExp = me.conf.tgexp * 1;

                                    if (exp >= needExp) return;
                                    if (oneExp + exp >= needExp) num = 1;
                                    else {
                                        num += 1;
                                        exp += oneExp;
                                        for (var i = 0; i < me.DATA.num - 1; i ++) {
                                    ...
                                    if(me.conf.color > 3){
                                        ...
                                        G.frame.alert.data({
                                            ...
                                            okCall: function () {
                                                var callback = me.data().callback;
                                                callback && callback({id:me.curTid,num:num,spid:me.curId});
                                                var lessnum = G.frame.beibao.DATA.shipin.list[me.curTid].num - G.frame.shipin_shengji.selectedData[me.curId];
                                                me.item.num.setString(lessnum);
                                            },
                                            richText: str,
