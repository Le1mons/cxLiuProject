 ...
 (function () {
    ...
    G.class.tiejiangpu_top = X.bView.extend({
        ...
        bindBtn:function () {
             ...
             me.ajax('equip_hecheng',[eid,str], function (s, d) {
                if(d.s == 1){
                    G.event.emit('sdkevent',{
                        event:'duanzao_equip',
                        data:{
                            consume:me._curData.need,
                            get:[{a:"equip",t:eid,n:str}],
                        }
                    });
                    try{
                      var need = [].concat(me._curData.need);
                      G.event.emit("leguXevent", {
                          type: 'track',
                          event: 'equip_hehceng',
                          data: {
                              equip_need: ["equip",need[0].t,str*need[0].n],
                              equip_prize:  ["equip",me._curData.id,str],
                              equip_jinbi: ["attr","jinbi",str*need[1].n],
                          }
                      });
                    }catch(e){
                        cc.error(e);
            ...
            //一键熔炼
            me.nodes.btn_yjrl.click(function () {
                me.ajax('equip_yjhecheng',[me._type,1],function (str,data) {
                    if(data.s == 1){
                        G.frame.equip_yjrl_tips.data({
                            type:me._type,
                            data:data.d
                        }).show();
                    }
