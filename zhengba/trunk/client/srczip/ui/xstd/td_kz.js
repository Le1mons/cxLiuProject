...
(function () {
    //大秘境-保存备战阵容
    G.class.xstd_kz = X.bView.extend({
        extConf:{
            maxnum:3,
        },
        ctor: function (type) {
        ...
        bindBTN: function () {
            ...
            me.nodes.btn_kz.click(function () {
                ...
                //     if(data.s == 1) {
                //         G.frame.xstd_fightRead.remove();
                //         G.frame.damijing.checkShow();
                //         G.hongdian.getData("watcher", 1);
                //     }
                // })

                G.frame.tdGame.DATA = {hero: arr};
                G.frame.tdGame.nodes.bg_kaishi.hide();
                G.frame.tdGame.gameStart();
                G.frame.xstd_fightRead.remove();
            });
        },
        onOpen: function () {
                ...
                if (type == ccui.Widget.TOUCH_MOVED) {
                    if (sender.data) {
                        ...
                        if (!cc.isNode(cloneItem)) {
                            cloneItem = G.class.shero(G.frame.tdGame.hero[sender.data]);
                            cloneItem.scale = .8;
                            cloneItem.tid = sender.data;
                            G.frame.xstd_fightRead.ui.addChild(cloneItem);
                            sender.hide();
                        }
        ...
        removeItem: function (tid) {
            ...
            for (var i = 0; i < itemArr.length; i++) {
                ...
                if (item.data && item.data == tid) {
                    var idx = X.arrayFind(G.frame.xstd_fightRead.top.selectedData, tid);
                    if (idx > -1) {
                        G.frame.xstd_fightRead.top.selectedData.splice(idx, 1);
                        G.frame.xstd_fightRead.top.removeGou(tid);
                    }

                    var child = G.frame.xstd_fightRead.top.getChildByTid(tid);
                    if (child) {
                        G.frame.xstd_fightRead.posSelect = G.frame.xstd_fightRead.ui.convertToNodeSpace(child.getParent().convertToWorldSpace(child.getPosition()));
                        G.frame.xstd_fightRead.posSelect.x += child.width / 2;
                    }
                    ...
                    G.frame.xstd_fightRead.playAniType = 'remove';
                    G.frame.xstd_fightRead.posSz = G.frame.xstd_fightRead.ui.convertToNodeSpace(layIco.getParent().convertToWorldSpace(layIco.getPosition()));
                    var itemClone = G.frame.xstd_fightRead.item = layIco.clone();
                    itemClone.setPosition(G.frame.xstd_fightRead.posSz);
        ...
        addItem: function (tid) {
            ...
            for (var i = 0; i < itemArr.length; i++) {
                ...
                if (!item.data) {
                    item.data = tid;
                    var layIco = item.nodes.panel_ico;
                    var wid = G.class.shero(G.frame.tdGame.hero[tid]);
                    wid.setAnchorPoint(0.5,1);
                    ...
                    G.frame.xstd_fightRead.playAniType = 'add';
                    G.frame.xstd_fightRead.posSz = G.frame.xstd_fightRead.ui.convertToNodeSpace(layIco.getParent().convertToWorldSpace(layIco.getPosition()));
                    G.frame.xstd_fightRead.posSz.x -= layIco.width / 2;
                    break;
                }
            }
            me.setZL();
        ...
        setZL: function () {
            ...
            var conf = G.frame.tdGame.enemy[0];
            ...
            for (var i = 0; i < me.itemArr.length; i ++) {
                ...
                if(!item.data) {
                    ...
                    num += G.frame.tdGame.hero[item.data].zhanli || 0;
                    var atk = G.frame.tdGame.hero[item.data].atk * conf.power * conf.frequent;
