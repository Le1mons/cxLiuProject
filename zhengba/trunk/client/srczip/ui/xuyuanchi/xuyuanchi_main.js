    ...
    var fun = X.bUi.extend({
            ...
            G.ajax.send("xuyuanchi_shuaxin", [me.type], function (d) {
                ...
                if (d.s == 1) {
                    if(me.mfsx){
                        ...
                        me.freshneed = me.conf.rfneed;
                    }
                    G.event.emit("sdkevent", {
                        event: "zhuanpan_refresh",
                        data:{
                            zhuanpanType:me.type,
                            consume:me.freshneed
                        }
                    });
                    me.refresh = 1;
                    ...
                    me.setRefreshTime();
                    if(me._time){
                        cc.isNode(me.ui) && me.ui.clearTimeout(me._time);
                        me._time = null;
                    }
                    me.refreshItems();
        ...
        lottery: function(type, is, isSuper, isAni){
            ...
            G.ajax.send("xuyuanchi_lottery", [me.type, type, isSuper], function (d) {
                ...
                if (d.s == 1) {
                    if(type == 1){
                        me.need = me.conf.oneneed;
                    }else if(type == 10){
                        me.need = me.conf.tenneed;
                    }
                    G.event.emit("sdkevent", {
                        event: "zhuanpan_run",
                        data:{
                ...
                (function (conf, i) {
                    ...
                    box.click(function (sender) {
                        ...
                        me.ajax("xuyuanchi_getprize", [sender.idx], function (str, data) {
                            if(data.s == 1) {
                                G.event.emit("sdkevent", {
                                    event: "zhuanpan_lingjiang",
                                    data:{
                                        szj_boxNum:sender.idx,
                                        get:data.d.prize
                                    }
                                });
                                G.frame.jiangli.data({
