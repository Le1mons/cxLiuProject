...
(function () {
    ...
    var fun = X.bUi.extend({
        ...
        setContents:function() {
            ...
            var keys = ['jingzhunpro','gedangpro','undpspro','dpspro','skilldpspro','baoshangpro','baojipro',"unbaojipro",'miankongpro','pojiapro',"pvpdpspro","pvpundpspro", 'undotdpspro',"realinjurypro"];
            ...
            for(var i=0;i<keys.length;i++){
                ...
                if(keys[i] == 'dpspro') {
                    var value = data.fkdpspro || 0;

                    if(value) {
                        num = parseInt(data[keys[i]]/10 + value / 10) + ("." + (data[keys[i]] + value)%10 + "%");
                    }
                }
                X.render({
