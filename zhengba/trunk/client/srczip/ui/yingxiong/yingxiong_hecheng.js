...
(function () {
    ...
    var fun = X.bUi.extend({
        ...
        setContents: function () {
            ...
            if(G.frame.yingxiong_xxxx.toZhongzu) type = G.frame.yingxiong_xxxx.toZhongzu;
        ...
        sortData: function (data) {
            ...
            for (var i in data) {
                obj[data[i]] = me.getNum(data[i]);
            }

            data.sort(function (a,b) {
                var confA = G.class.hero.getById(a);
                var confB = G.class.hero.getById(b);
                if(obj[a].can != obj[b].can) {
                    return  obj[a].can > obj[b].can ? -1 : 1;
        ...
        setItem: function (ui, data) {
            ...
            ui.touch(function (sender, type) {
                if (type == ccui.Widget.TOUCH_NOMOVE) {
                    ...
                    me.curConf = sender.heroData.conf;
                    me.selectedData = {};
                    me.handleHeros(me.curId);
                    me.setTop();
                }
        ...
        setPer: function(ui) {
            var me = this;
            var needConf = G.class.hero.getHcNeed(ui.data);
            var needNum = 0;
            var needArr;
            if(needConf.delhero[1]) {
                needArr = [needConf.mainhero,needConf.delhero[0],needConf.delhero[1],needConf.chkhero[0]];
            }else {
                needArr = [needConf.mainhero,needConf.delhero[0],needConf.chkhero[0],needConf.chkhero[1]];
            }
            for (var i in needArr) {
                needNum += (needArr[i].n || needArr[i].num);
            }
            var str = me.canObj[ui.data].have + "/" + needNum;
            ui.wid.setHP(me.canObj[ui.data].have / needNum * 100, true, str);
        },
        getNum: function(data) {
            var me = this;
            var needConf = G.class.hero.getHcNeed(data);
            var needNum = 0;
            var needArr;
            if(needConf.delhero[1]) {
                needArr = [needConf.mainhero,needConf.delhero[0],needConf.delhero[1],needConf.chkhero[0]];
            ...
            var have = me.setTC(needArr, G.class.hero.getById(data), true, true);
            var obj = {};
            obj.have = have;
            obj.can = have >= needNum;
            return obj;
        },
        ...
        setTC: function(arr, conf, isT, isCheck) {
            var me = this;
            var xzArr = [];
            var hero = [];
            var selectedData = me.selectedData;

            hero.push(me.getHidArr(arr[0].t, function (dd) {
                dd.sort(function (a, b) {
                    var da = G.DATA.yingxiong.list[a];
                    var db = G.DATA.yingxiong.list[b];

                    return da.lv > db.lv ? -1 : 1;
                });
            }, true));

            hero.push(me.getHidArr(arr[1].t, function (dd) {
                dd.sort(function (a, b) {
                    var da = G.DATA.yingxiong.list[a];
                    var db = G.DATA.yingxiong.list[b];
            ...
            (arr[2].samezhongzu || arr[2].jichuzhongzu) ? hero.push(
                me.getHeroArr(arr[2].samezhongzu ? conf.zhongzu : [1, 2, 3, 4], arr[2].star, isCheck))
                : hero.push(me.getHidArr(arr[2].t, function (dd) {
                dd.sort(function (a, b) {
                    var da = G.DATA.yingxiong.list[a];
                    var db = G.DATA.yingxiong.list[b];
            ...
            hero.push(me.getHeroArr(arr[3].samezhongzu ? conf.zhongzu : [1, 2, 3, 4], arr[3].star, isCheck, true));

            if(!isT) {
                for (var i = 0; i < arr.length; i ++) {
                    selectedData[i] = [];
                    for (var j in hero[i]) {
                        if(!X.inArray(xzArr, hero[i][j])) {
                            selectedData[i].push(hero[i][j]);
                            xzArr.push(hero[i][j]);
                        }
                        if(selectedData[i].length == arr[i].n || selectedData[i].length == arr[i].num) {
                            break;
                        }
                    }
                }
                ...
                for (var n = 0; n < arr.length; n ++) {
                    hc[n] = [];
                    for (var j in hero[n]) {
                        if(hero[n][j] && !X.inArray(haveArr, hero[n][j])) {
                            hc[n].push(hero[n][j]);
                            haveArr.push(hero[n][j]);
                        }
                        if(hc[n].length == arr[n].n || hc[n].length == arr[n].num) {
                            break;
                        }
                    }
                }
                return haveArr.length;
            }
        },
        getHidArr: function(hid, sortCall, isCheck) {
            var arr = [];
            ...
            if(isCheck) {
                ...
                for (var i in data) {
                    if(data[i].hid == hid && !X.inArray(G.DATA.yingxiong.jjchero, i) && !data[i].islock) {
                        arr.push(i);
                    }
                }
            }

            sortCall && sortCall(arr);
            return arr;
        },
        getHeroArr: function(zz, star, isCheck, isNoLock) {
            var arr = [];
            var data = G.DATA.yingxiong.list;
            zz = [].concat(zz);
            ...
            if(isCheck) {
                for (var i in data) {
                    if(X.inArray([zz], data[i].zhongzu) && data[i].star == star) {
                        arr.push(i);
                    }
                }
            } else {
                if(!isNoLock) {
                    for (var i in data) {
                        if(X.inArray([zz], data[i].zhongzu)
                            && data[i].star == star
                            && !X.inArray(G.DATA.yingxiong.jjchero, i)
                            && !data[i].islock
                        ) {
                            arr.push(i);
                        }
                    ...
                    for (var i in data) {
                        if(X.inArray([zz], data[i].zhongzu)
                            ...
                            && !X.inArray(G.DATA.yingxiong.jjchero, i)
                            && !data[i].islock
                            && G.class.herostarup.getUpStarNum(data[i].hid) * 1 < 10
                        ) {
                            arr.push(i);
                        }
                    }
            ...
            //     var da = G.DATA.yingxiong.list[a];
            //     var db = G.DATA.yingxiong.list[b];
            //     da.starnum = G.class.herostarup.getUpStarNum(da.hid);
            //     db.starnum = G.class.herostarup.getUpStarNum(db.hid);
            //
            //     if(da.starnum != db.starnum) {
            //         return da.starnum * 1 < db.starnum * 1 ? -1 : 1;
            //     } else if(da.lv != db.lv) {
            //         return da.lv < db.lv ? -1 : 1;
            //     } else {
            //         return da.hid < db.hid ? -1 : 1;
            //     }
            // });
        ...
        setTop: function () {
            ...
            if(X.inArray(me.canArr, curId)) {
                me.setTC(extNeed, me.curConf, false, false);
                for (var i = 0; i < me.extArr.length; i ++) {
                    me.TChero(me.extArr[i], me.selectedData[i]);
                }
                ...
                G.ajax.send('hero_hecheng',[me.curId,obj],function(d) {
                    ...
                    if(d.s == 1) {
                        ...
                        if (def) {
                            ...
                            for (var k in def) {
                                G.DATA.yingxiong.jjchero.push(def[k]);
                            }
                            X.cacheByUid('fight_zyjjc', def);
                        }
        ...
        setMaterial: function (parent,need,idx) {
            ...
            var widget = G.class.shero_extneed(need, {hid:need.t || me.curId}, true, me.nodes[nameArr[idx]]);
            ...
            widget.click(function(sender,type){
                ...
                G.frame.ui_tip_xuanze.data({
                    ...
                    callback:callback,
                    noLock: sender.index == 0 ? true : false
                }).show();
            });
        },
        TChero: function(node, arr) {

            node.txt_num.setString(X.STR('{1}/{2}', arr.length, node.need.num));
            node.setEnabled(arr.length >= node.need.num);
            node.setjia(true);
        },
        //获得所有被选择的英雄的数组
