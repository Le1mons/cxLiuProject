...
//                  var dataB = X.clone(G.DATA.yingxiong.list[b]);
            ...
            2: {
                data: function (star) {
                    ...
                    if (star == 0) {
                        for(var i = 0; i < keys.length; i ++){
                            var tid = keys[i];
                            data[tid].lv && data[tid].lv > 1 && arr.push(keys[i]);
                        }
                    } else {
                        for (var i = 0; i < keys.length; i++) {
                            var tid = keys[i];
                            var heroData = data[tid];
                            if (heroData.zhongzu == star && heroData.lv && heroData.lv > 1) {
                                arr.push(tid);
                            }
                        }
                    }
        ...
        bindBTN: function () {
            ...
            me.nodes.btn_tishi.touch(function (sender, type) {
                if (type == ccui.Widget.TOUCH_ENDED) {
                    if (me._type == 1) {
                        ...
                        var sData = G.frame.yingxiong_fenjie.top.selectedData;
                        if(sData.length < 1){
                            return G.tip_NB.show("请放入需要重生的英雄");
                        }
                        me.ajax("hero_reborn", [me.selectedData[0], true], function (str, data) {
                            if (data.s == 1) {
                                G.frame.jiangliyulan.data({
                                    prize:data.d.prize,
                                }).show();
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        if(G.DATA.yingxiong.list[tid].star < 5 && !X.inArray(lockArr,tid)){
                            ...
                            if (obj && cc.sys.isObjectValid(obj[0])) {
                                ...
                                // me.addItem(tid);
                                if(me.selectedData.length < cont){
            ...
            var need = G.gc.herocom.reborn[0];
            ...
            me.nodes.btn_3.click(function (sender) {
                if (me.selectedData.length == 0) return G.tip_NB.show(L("YX_CHONGSHNEG_TIP"));
                if (G.class.getOwnNum(need.t, need.a) < need.n) return G.tip_NB.show(G.class.getItem(need.t, need.a).name + L("BUZU"));
                ...
                G.frame.alert.data({
                    ...
                    okCall: function () {
                        ...
                        me.oldlv = G.DATA.yingxiong.list[me.selectedData[0]].lv;
                        me.ajax("hero_reborn", [me.selectedData[0], false], function (str, data) {
                            if (data.s == 1) {
                                G.event.emit('sdkevent',{
                                    event:'hero_reborn',
                                    data:{
                                        fenjie_zhenying:G.DATA.yingxiong.list[me.selectedData[0]].zhongzu,
                                        fenjie_Lv:me.oldlv,
                                        consume:need,
                                        get:data.d.prize
                                    }
        ...
        changeType: function (type) {
            ...
            me._type = type;
            me.selectedData = [];
            me.nodes.btn_1.setVisible(type == 1);
            me.nodes.btn_2.setVisible(type == 1);
            me.nodes.btn_3.setVisible(type == 2);
            me.setContents();
        },
        ...
        setItem: function (ui, data) {
            ...
            if (X.inArray(me.lockArr,data) || G.DATA.yingxiong.list[data].islock || (heroData.star >= 7 && me._type == 1)) {
                imgSuo.show();
                    ...
                    if (X.inArray(me.selectedData, sender.data)) {
                        ...
                        if (me.selectedData.length >= G.frame.yingxiong_fenjie.bottom.extConf.maxnum) {
                            if(me.kuaisu){
                                ...
                                G.tip_NB.show(X.STR(L('YX_SX_XZ_FULL'), G.frame.yingxiong_fenjie.bottom.extConf.maxnum));
                            }
                            return;
                        }
