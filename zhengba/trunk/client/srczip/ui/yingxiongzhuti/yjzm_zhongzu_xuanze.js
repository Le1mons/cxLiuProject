...
(function () {
    //永杰之门-种族-选择
    G.class.yjzm_zhongzu_xuanze = X.bView.extend({
        extConf:{
            fight:{
                data: function (type) {
                    var data = G.frame.yjzm_setDef.DATA.hero;
                    ...
                    if (type == 0) {
                        ...
                        for (var i = 0; i < keys.length; i++) {
                            var hid = keys[i];
                            var heroData = G.class.hero.getById(hid);
                ...
                getSort: function (arr) {
                    ...
                    for(var i = 0; i < arr.length; i ++){
                        heroData.push(G.frame.yjzm_setDef.DATA.hero[arr[i]]);
                    }
        ...
        setItem: function (ui, data) {
            ...
            X.autoInitUI(ui);
            var heroData = G.frame.yjzm_setDef.DATA.hero[data];
            ui.setName(data);
            ...
            ui.touch(function (sender, type) {
                if (type == ccui.Widget.TOUCH_NOMOVE){
                    ...
                    G.frame.yjzm_setDef.posSelect = G.frame.yjzm_setDef.ui.convertToNodeSpace(sender.getParent().convertToWorldSpace(sender.getPosition()));
                    if (X.inArray(me.selectedData, sender.data)) {
                        G.frame.yjzm_setDef.posSelect.x += sender.width / 2;
                        sender.finds('widget').setGou(false);
                        me.selectedData.splice(X.arrayFind(me.selectedData,sender.data),1);
                        G.frame.yjzm_setDef.bottom.removeItem(sender.data);
                    } else {
                        if (me.selectedData.length >= 6) {
                            G.tip_NB.show(L('YX_FIGHT_XZ_FULL'));
                            for (var i = 0; i < G.frame.yjzm_setDef.bottom.panel_yx.children.length; i ++) {
                                var chr = G.frame.yjzm_setDef.bottom.panel_yx.children[i];
                        ...
                        G.frame.yjzm_setDef.bottom.addItem(sender.data);

                        if (cc.isNode(G.frame.yjzm_setDef.item)) {
                            G.frame.yjzm_setDef.item.stopAllActions();
                            G.frame.yjzm_setDef.item.removeFromParent();
                        }
                        var itemClone = G.frame.yjzm_setDef.item = sender.clone();
                        itemClone.finds('gou').hide();
                        itemClone.setPosition(G.frame.yjzm_setDef.posSelect);
