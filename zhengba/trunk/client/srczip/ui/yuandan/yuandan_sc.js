...
(function () {
    ...
    var fun = X.bUi.extend({
        ...
        setItem: function(ui, data, idx){
            ...
            var buyNum = G.DATA.yuandan.store.length > idx ? G.DATA.yuandan.store[idx] : 0;
            var sellOut = data.num <= buyNum;
            var hasSale = data.sale && data.sale != 100;
            var synum = data.num - buyNum;
            X.render({
                ...
                wz1: function(node){
                    ...
                    if(P.gud.vip < data.vip){
                        node.setString(X.STR(L('yuandan_vipxg'), data.vip));
                        X.enableOutline(node,cc.color('#0e9409'), 2);
                    }else{
                        if(data.refresh == 0){
                            node.setString(X.STR(L('yuandan_store_xg1'), data.num - buyNum));
                            X.enableOutline(node,cc.color('#0e9409'), 2);
                        }else if(data.refresh == 1){
                            node.setString(X.STR(L('yuandan_store_xg2'), data.num - buyNum));
                ...
                text_zk: function(node){
                    if(hasSale && !sellOut){
                        node.setString(X.STR(L('yuandan_store_sale'), data.sale / 10));
                        X.enableOutline(node,cc.color('#CB630A'), 2);
                    }else{
                        node.setString('');
                    }
            ...
            ui.touch(function (sender, type) {
                if (type === ccui.Widget.TOUCH_NOMOVE) {
                    ...
                    if(P.gud.vip < data.vip){
                        G.tip_NB.show(X.STR(L('yuandan_tip_vipxg'), data.vip));
                        return;
                    }
                    ...
                    G.frame.buying.data({
                        ...
                        need: itemData.need,
                        maxNum: synum < 0 ? 0 : synum,
                        callback: function (num) {
                            G.DAO.yuandan.store(idx, num, function(dd) {
    ...
    G.frame[ID] = new fun('event_yuandan_zqss.json', ID);
})();