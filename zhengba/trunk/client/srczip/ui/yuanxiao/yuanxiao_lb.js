...
(function () {
    ...
    var fun = X.bUi.extend({
        ...
        setTable: function () {
            var me = this;
            var conf = G.gc.yuanxiao.libao;
            var data = [];
            cc.each(conf, function (obj, id) {
                var _obj = JSON.parse(JSON.stringify(obj));
                _obj.id = id;
                _obj.buyNum = G.frame.yuanxiao.DATA.myinfo.gotarr.libao[_obj.proid] || 0;
        ...
        setItem: function (ui, data) {
            ...
            X.render({
                libao_mz: function (node) {
                    node.setString(data.name || 'sbysr');
                ...
                btn_gm: function (node) {
                    ...
                    node.click(function () {
                        G.event.once('paysuccess', function(arg) {
                            ...
                            if (!G.frame.yuanxiao.DATA.myinfo.gotarr.libao[data.id]) {
                                G.frame.yuanxiao.DATA.myinfo.gotarr.libao[data.id] = 0;
                            }
                            G.frame.yuanxiao.DATA.myinfo.gotarr.libao[data.id] += 1;
                            me.setTable();
                        });
    ...
    G.frame[ID] = new fun('yuanxiaodenghui_tip_yxlb.json', ID);
})();