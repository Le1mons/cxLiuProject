...
(function () {
    ...
    var fun = X.bUi.extend({
                ...
                if (data.s === 1) {
                    if(port) {//刷新
                        ...
                        if(me.DATA.num == 0){
                            ...
                            var need = [{a:"attr",t:"rmbmoney",n:15}];
                        }
        ...
        bindUI: function () {
            ...
            var talkNode = me.nodes.panel_rw;
            talkNode.setTouchEnabled(true);
            talkNode.click(function () {
                me.action.play("qipao", false);
                me.setTalk();
            });
        },
        ...
        _setItem: function (item, data,idx) {
            ...
            if (data.need[0].t == "jinbi") item.nodes.img_tjbq.show();
            ...
            G.frame.alert.data({
                ...
                okCall: function () {
                    me.ajax('zahuopu_buy',[item.idx],function(str ,data){
                        if (data.s === 1) {
                            var need = me.DATA.itemlist[item.idx].need;
                            G.event.emit("sdkevent", {
                                event: "shopBuy",
                                data:{
                                    shopType:"zahuopu",
        ...
        // },
        setTalk: function () {
            var str = X.arrayRand(G.gc.dianjin.talk);
            this.nodes.text_3.setString(str);
        },
        onAniShow: function () {
            var me = this;
