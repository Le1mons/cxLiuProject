...
(function () {
    ...
    var fun = X.bUi.extend({
        ...
        setTable: function () {
            var me = this;
            var conf = G.gc.zhishujie.duihuan;
            var data = [];
            cc.each(conf, function (obj, id) {
                var _obj = JSON.parse(JSON.stringify(obj));
                _obj.id = id;
                _obj.buyNum = G.frame.zhishujie_main.DATA.myinfo.duihuan[_obj.id] || 0;
                    ...
                    node.click(function () {
                        G.frame.buying.data({
                            ...
                            callback: function (num) {
                                me.ajax('planttrees_duihuan', [data.id, num], function (str, _data) {
                                    if (_data.s == 1) {
                                        G.frame.jiangli.data({
                                            prize: _data.d.prize
                                        }).show();
                                        if (!G.frame.zhishujie_main.DATA.myinfo.duihuan[data.id]) {
                                            G.frame.zhishujie_main.DATA.myinfo.duihuan[data.id] = 0;
                                        }
                                        G.frame.zhishujie_main.DATA.myinfo.duihuan[data.id] += num;
                                        me.setTable();
                                        me.showAttr();
                                    }
                                });
        ...
        showAttr: function () {
            var me = this;
            var need = G.gc.zhishujie.duihuanNeed[0];

            var node = new ccui.ImageView(G.class.getItemIco(need.t), 1);
            var rh = X.setRichText({
                str: X.STR(L("YYXG"), "<font node=1></font><font color=#1c9700>" + X.fmtValue(G.class.getOwnNum(need.t, need.a)) + "</font>"),
    ...
    G.frame[ID] = new fun('zhishujie_zsdh.json', ID);
})();