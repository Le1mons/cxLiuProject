    ...
    for idx,item in enumerate(reader):
        if idx==0:
            ...
            try:
                ...
                s = s.replace(chr(1),'')
                s = s.decode('gbk')
                _js = json.loads(s, object_pairs_hook=OrderedDict)
            except:
                print '[ERROR]!!!!!!!!!!!!!!!'
                print 'file =',f
                print 'line =',idx
            ...
            if 'group' in cnf[f]:
                ...
                if res==None:res=OrderedDict()

                __sgk = None
                if 'subgroup' in cnf[f]:
                    __sgk = (item[keyDict[ cnf[f]['subgroup'] ]]).decode('gbk')
                
                if 'key' in cnf[f]:
                    if not __gk in res :
                        res[__gk] = OrderedDict()
                    
                    if __sgk!=None and __sgk not in res[__gk]:
                        res[__gk][__sgk] = OrderedDict()
                    
                    __k = (item[keyDict[ cnf[f]['key'] ]]).decode('gbk')

                    if __sgk!=None:
                        res[__gk][__sgk][__k] = _js
                    else:
                        res[__gk][__k] = _js
                else:
                    if not __gk in res :
