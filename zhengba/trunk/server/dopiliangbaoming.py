...
def creatPlayer(data, **args):
    ...
    uid = str(_sidx) + '_' + str(_oid)
    uuid = str(_sidx) + g.C.getUUIDByDBID(str(_oid))
    _uinfo = {
        'uid': uid,
        'uuid':uuid
    }
...
def doBaoming(num):
    ...
    if len(_data) <= num:
        ...
        minzhanli = 1065
        userNum = g.mdb.count('userinfo') + 1
        for i in xrange(num - len(_data)):
            ...
            sampleData['binduid'] = 'robot' + str(userNum)
            sampleData['sex'] = random.randint(0,1)
            sampleData['name'] = 'robot' + str(userNum)
    ...
    for _user in _data:
        ...
        if heroNum < 21:
            # 随机发送12个hero
            randHid = random.sample(hidList, 21 - heroNum)
            for hid in randHid:
                _newHero = g.m.herofun.addHero(_user['uid'], hid)
                _heroData = g.m.herofun.reSetHeroBuff(_user['uid'], _newHero['tid'])

        heroList = g.mdb.find('hero', {'uid': _user['uid']}, fields={'_id': 1}, sort=[['lv', -1]], limit=21)
        _heroData = [{}, {}, {}]
