...
def doproc(conn, data):
    ...
    _isapplyList = g.getAttrByCtype(uid, 'friend_isapply', default=[])
    # 不可重复申请
    if _toUid in _isapplyList:
        _res['s'] = -5
    ...
    if g.m.crosscomfun.chkIsThisService(_toUid):
        ...
        _crossApply = g.crossDB.find1('cross_friend', {'uid': _toUid}, fields=['_id', 'apply']) or {'apply': []}
        _applyList = _crossApply['apply']
        ...
        g.crossDB.update('cross_friend', {'uid': _toUid},{'$addToSet': {'apply': uid}}, upsert=True)

    g.mdb.update('friend',{'uid':uid},{'$pull':{"tuijian":_toUid}},upsert=True)
    g.setAttr(uid, {'ctype': 'friend_isapply'}, {'$push':{'v':_toUid}})
...
if __name__ == '__main__':
    uid = g.buid("xuzhao")
    g.debugConn.uid = uid
    print doproc(g.debugConn,["90_5cd29d509dc6d6730af92cc0"])