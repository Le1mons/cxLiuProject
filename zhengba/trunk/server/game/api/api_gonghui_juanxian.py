...
def doproc(conn,data):
    ...
    # 战旗登陆任务
    g.event.emit("FlagTask", uid, '102')

    # 王者招募任务监听
    g.event.emit("wzzmtask", uid, "117")

    # 记录贡献
    g.m.gonghuifun.setGuildContri(uid, _ghid, _gcon['juanxian'][_jxtype]['prize'])
    # 周年庆
    g.event.emit('ANNIVERSARY', uid, '3')
    # 圣诞活动
    g.event.emit('shengdan', uid, {'task': ['2003']})
    return _res
