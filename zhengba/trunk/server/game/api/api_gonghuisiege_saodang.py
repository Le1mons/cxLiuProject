...
def doproc(conn, data):
    ...
    _fightlog = []
    _fightnum = 0
    for _idx, info in enumerate(_pklist):
        ...
        if not _enemyUid.startswith('NPC'):
            ...
            if not _enemyInfo:
                _res['s'] = -6
                _res['errmsg'] = g.L('gonghuisiege_fight_res_-6')
                return _res
        ...
        _signdata = {}
        for role, sign in _fightRes["signdata"].items():
            if "hid" not in _fightRes["fightres"][role]:
                continue
            _signdata[role] = {}
            _signdata[role].update(sign)
            _signdata[role]["hid"] = _fightRes["fightres"][role]["hid"]
            _signdata[role]["side"] = _fightRes["fightres"][role]["side"]

        _pklist[_idx]['winside'] = _winside
        _fightlog.append({"winside": _winside, "signdata": _signdata, "headdata": _fightRes['headdata'], })
        ...
        if not _winside:
            ...
            if _enemyInfo:
                ...
                _setEnemyData = {}
                _setEnemyData["alljifen"] = _enemyAllJiFen = _enemyInfo["alljifen"] - 2 if _enemyInfo["alljifen"] - 2 > 0 else 0
                _setEnemyData["jifeninfo"] = _enemyInfo["jifeninfo"]
                _setEnemyData["jifeninfo"][_cityid] = _enemyInfo["jifeninfo"].get(_cityid, 0) - 2 if _enemyInfo["jifeninfo"].get(_cityid, 0) - 2 > 0 else 0
                # 设置对方数据
                g.m.gonghuisiegefun.setUserData(_enemyUid, _setEnemyData)
                # 设置对方集团的总积分扣除
                g.m.gonghuisiegefun.setGongHhuiCityJiFen(_enemyUid, _cityid, _enemyInfo["ghid"], -2)

            _jifenInfo = {'jifen': _myInfo["alljifen"] + _addjifen, 'add': _addjifen, 'reduce': -2, 'rivaljifen': _enemyAllJiFen,'win_uid': uid}
            _jifeninfolist.append({'jifen': _myInfo["alljifen"] + _addjifen, 'add': _addjifen, 'reduce': -2, 'rivaljifen': _enemyAllJiFen,'win_uid': uid})
        else:
            _addjifen = 1
            # 设置日志N
            _enemyAllJiFen = 0
            ...
            # 如果不是npc,减少对应对方对应的积分和日志数据
            if _enemyInfo:
                _enemyAllJiFen = _enemyInfo["alljifen"]
                _enemylog = ["2", gud["ghname"], gud["name"], _pklist[_idx]["headdata"]["name"], _nt]
            _winGh = _pklist[_idx]["headdata"].get("ghid", "npc")
            _jifenInfo = {'jifen': _myInfo["alljifen"] + _addjifen,'add':0,'reduce':_addjifen,'rivaljifen':_enemyAllJiFen,'win_uid':_enemyUid}
            _jifeninfolist.append({'jifen': _myInfo["alljifen"] + _addjifen,'add':0,'reduce':_addjifen,'rivaljifen':_enemyAllJiFen,'win_uid':_enemyUid})
    ...
    _resData["jifeninfolist"] = _jifeninfolist
    # # 获取城市战力的排行
    _resData["cityrank"] = g.m.gonghuisiegefun.getCityRank(uid, _cityid)
    _resData["fightlog"] = _fightlog
...
if __name__ == '__main__':
    a = g.getGud(g.buid('0'))
    g.debugConn.uid = g.buid('490000')
    _data = [1, {"1":"5fe6e3319dc6d6695ae227bb"}]
