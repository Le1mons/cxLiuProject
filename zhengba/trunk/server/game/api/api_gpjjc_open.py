...
def doproc(conn, data):
    ...
    # 获取排行数据
    _rankinfo = g.m.rankfun.getGpjjcRank(uid=uid,limit=10)

    _resData["myinfo"] = _myinfo
    _resData["rankinfo"] = _rankinfo
    _resData["libao"] = g.m.gongpingjjcfun.getLiBaoInfo(uid)
    # 匹配数据
    ...
    if _resData["pipeiinfo"]:
        ...
        if _resData["pipeiinfo"]["rivaluid"] != "npc":
            _resData["rivalpipeiinfo"] = g.m.gongpingjjcfun.getPipeiData(_resData["pipeiinfo"]["rivaluid"],
                                                                         keys="fightdata,headdata,_id")
        else:
    ...
    _resData["todayfightnum"] = g.CROSSATTR.getPlayAttrDataNum(uid, "gpjjc_todayfightnum")
    _res["d"] = _resData
