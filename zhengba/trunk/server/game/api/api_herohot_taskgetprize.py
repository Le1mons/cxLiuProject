...
def _checkCond(uid, data):
    ...
    # 判断是否已经参与过海选
    if _myinfo["task"].get(_taskid, 0) < _con["task"][_taskid]["pval"]:
        _chkData['s'] = -3
        _chkData['errmsg'] = g.L('global_noopen')
        return _chkData

    # 判断任务是否已经领取
    if  _taskid in _myinfo["taskrec"]:
        _chkData['s'] = -4
        _chkData['errmsg'] = g.L('global_noopen')
        return _chkData

    _prize = _con["task"][_taskid]["prize"]
...
def doproc(conn, data):
    ...
    _setData = {}
    _myinfo["taskrec"].append(_taskid)
    _setData["taskrec"] = _myinfo["taskrec"]

    # 设置购买数据
    g.m.herohotfun.setUserData(uid, _openinfo["hdid"], _setData)
    # 发奖
    _sendData = g.getPrizeRes(uid, _prize, {'act': 'herohot_taskgetprize', "taskid": _taskid})
