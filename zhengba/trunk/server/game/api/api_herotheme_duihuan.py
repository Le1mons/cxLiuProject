...
def _checkCond(uid, data):
    ...
    _con = g.m.herothemefun.getCon()['duihuan'][_id]
...
def doproc(conn, data):
    ...
    # 删除道具
    _send = g.delNeed(uid, _chkData['need'], 0, {'act': 'herotheme_duihuan', 'id':_id})
    g.sendChangeInfo(conn, _send)

    _prize = g.mergePrize(_prize * _num)
    # 设置任务领奖
    g.m.herothemefun.setData(uid, _chkData['hdid'], {'duihuan': _data["duihuan"]})

    _send = g.getPrizeRes(uid, _prize, {'act': 'herotheme_duihuan', 'id':_id})
    g.sendChangeInfo(conn, _send)
