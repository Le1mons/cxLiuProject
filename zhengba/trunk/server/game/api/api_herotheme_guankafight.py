...
def _checkCond(uid, data):
    ...
    if _openDay < _guankaOpen[str(_guankaCon["page"])]:
        _chkData['s'] = -1
        _chkData['errmsg'] = g.L('global_noopen_1')
        return _chkData
...
def doproc(conn, data):
    ...
    for pos, hid in _fightData.items():
        ...
        # 获取英雄数据
        _heroInfo = g.m.herothemefun.makeHeroBuff(_herodata)
    ...
    if _winside == 0:
        ...
        if _idx not in _data["guankarec"]["win"]:
            ...
            _sendData = g.getPrizeRes(uid, _prize, {'act':'herotheme_fight', 'idx': _idx})
            g.sendChangeInfo(conn, _sendData)
    # g.event.emit('zhounian3', uid, "3")
    _resData["myinfo"] = g.m.herothemefun.getData(uid, _hdid, keys='guankarec,task')
    _res['d'] = _resData
    return _res
...
if __name__ == '__main__':
    uid = g.buid("1")
    g.debugConn.uid = uid
    print doproc(g.debugConn, data=['1', {1: "61036", 2: "45046", 3: "41076", 4: "32076", 5: "32056", 6: "14066"}])