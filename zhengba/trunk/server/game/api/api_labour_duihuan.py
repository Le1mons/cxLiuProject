...
def _checkCond(uid, data):
    ...
    _con = g.m.labourfun.getCon()["duihuan"][_id]
...
def doproc(conn, data):
    ...
    # 删除道具
    _send = g.delNeed(uid, _chkData['need'], 0, {'act': 'labourhd_duihuan', 'id':_id})
    g.sendChangeInfo(conn, _send)

    _prize = g.mergePrize(_prize * _num)
    # 设置任务领奖
    g.m.labourfun.setData(uid, _chkData['hdid'], {'duihuan': _data["duihuan"]})

    _send = g.getPrizeRes(uid, _prize, {'act': 'labourhd_duihuan', 'id':_id})
    g.sendChangeInfo(conn, _send)

    _res['d'] = {'prize': _prize}
    _data = g.m.labourfun.getData(uid, _chkData['hdid'])
    _res['d']['myinfo'] = _data
    return _res
