...
def _checkCond(uid, data):
    ...
    _con = g.m.midautumnfun.getCon(_hd['data']['con'])['workshop'][_idx]

    # 兑换超过次数
    if _data['workshop'].get(str(_idx), 0) + _num > _con['num']:
        _res['s'] = -2
...
def doproc(conn, data):
    ...
    _send = g.delNeed(uid, _chkData['need'], 0)
    g.m.midautumnfun.setData(uid, _chkData['hdid'], {'$inc': {'workshop.{}'.format(data[0]): _chkData['num']}})

    _prize = _chkData['prize']
    _s = g.getPrizeRes(uid, _prize, {'act': 'midautumn_workshop','args':data})
