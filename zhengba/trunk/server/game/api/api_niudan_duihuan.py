...
def doproc(conn, data):
    ...
    _con = _chkData["hdinfo"]["data"]['duihuan'][_id]
    ...
    # 删除道具
    _send = g.delNeed(uid, _chkData['need'], 0, {'act': 'niudanhd_duihuan', 'id':_id})
    g.sendChangeInfo(conn, _send)

    _prize = g.mergePrize(_prize * _num)
    # 设置任务领奖
    g.m.niudanfun.setData(uid, _chkData['hdid'], {'duihuan': _data["duihuan"]})

    _send = g.getPrizeRes(uid, _prize, {'act': 'niudanhd_duihuan', 'id':_id})
    g.sendChangeInfo(conn, _send)

    _res['d'] = {'prize': _prize}
    _data = g.m.niudanfun.getData(uid, _chkData['hdid'])
    _res['d']['myinfo'] = _data
    return _res
