...
def doproc(conn, data):
    ...
    _prize = list(_con['commonprize'])[_idx]["p"]
    _data = _chkData["data"]

    # 记录任务领奖
    _data["commonprize"].append(_idx)
    _setData = {}
    _setData["commonprize"] = _data["commonprize"]

    # 设置数据
    g.m.planttreesfun.setData(uid, _chkData['hdid'], _setData)
    _send = g.getPrizeRes(uid, _prize, {'act': 'planttreeshd_comonprize', 'idx': _idx})
