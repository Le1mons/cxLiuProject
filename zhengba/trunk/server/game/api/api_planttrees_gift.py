...
def _checkCond(uid, data):
    ...
    # g.m.planttreesfun.getHelpData
    _con = g.m.planttreesfun.getCon()
    _gift = _data["gift"]
    _maxNum = _con["giftmaxnum"]
    # 如果是一键
    if _fromUid == "all":
        # 获取好友列表
        _friendList = g.m.friendfun.getFriendList(uid)
        if len(_friendList) <= len(_gift):
            _chkData['s'] = -3
            _chkData['errmsg'] = g.L('planttrees_res_-4')
            return _chkData
        _addUid = []
        # 循环加入
        for _friend in _friendList:
            if _friend in _gift:
                continue
            _addUid.append(_friend)

    else:
        if _fromUid in _gift:
            _chkData['s'] = -2
            _chkData['errmsg'] = g.L('planttrees_res_-4')
            return _chkData
...
def doproc(conn, data):
    ...
    _maxNum = _con["giftmaxnum"]
    # 超过赠送上限
    if len(_data["gift"]) >= _maxNum:
        _addVal = 0
    else:
        _addNum = min(_maxNum - len(_data["gift"]), len(_addUid))
        _addVal = _addNum * _con["giftval"]

    _setData = {}
    _data["gift"] += _addUid
    _setData["gift"] = _data["gift"]
    _lessAdd = _addVal
    ...
    for pos in xrange(8):
        ...
        if info["v"] + _lessAdd >= _con["maxval"]:
            ...
            _addData["p"] = _pColor
            # _data["fruitrec"][str(_pColor)] = _data["fruitrec"].get(_pColor, 0) + 1
        else:
    ...
    for _uid in _addUid:
        g.m.planttreesfun.setHelpData(_uid, _hdid, [uid])
