...
def doproc(conn, data):
    ...
    _cacheRank = g.crossMC.get("planttrees_rank_{}".format(_chkData["hdid"]))
    if _cacheRank:
        ...
        _rankList = []
        _uid2rank = {}
        _list = g.crossDB.find('crossplayattr', {'k': _chkData["hdid"], "ctype":"planttrees_5num"}, fields=['_id', 'v', "uid"], sort=[['v', -1],  ['lasttime', 1]], limit=50)
        _rank = 1
        ...
        if len(_rankList) > 0:
            g.crossMC.set("planttrees_rank_{}".format(_chkData["hdid"]), {"list": _rankList, 'uid2rank': _uid2rank}, 60)

    _myRank = -1
    _myData = g.crossDB.find1('crossplayattr', {'k': _chkData["hdid"], "ctype":"planttrees_5num", "uid": uid},
                              fields=['_id', 'v'])
    _num = 0
    if _myData:
        _num = _myData['v']
    if uid in _uid2rank:
        ...
        _myRank = g.crossDB.count('crossplayattr', {'k': _chkData["hdid"], "ctype":"planttrees_5num", "v":{"$gt": _num}}) + 1
...
if __name__ == '__main__':
    ...
    g.debugConn.uid = g.buid('jingqi_2004071032424570')
