...
def _checkCond(uid,data):
    ...
    _taskCon = _con["foevertask"][_idx]
    # 今天已经没有挑战次数了
    if _data["toplayer"] < _taskCon["pval"]:
        _chkData['s'] = -1
        _chkData['errmsg'] = g.L('hltb_fight_res_-9')
        return _chkData

    _recList = g.m.shilianztfun.getFoeverTaskRec(uid)
    # 判断是否通关
    if _idx in _recList:
        _chkData['s'] = -1
        _chkData['errmsg'] = g.L('hltb_fight_res_-9')
...
def doproc(conn, data):
    ...
    _prizeData = g.getPrizeRes(uid, _prize,  {'act': "shilianzt_gefoeverprzie", "prize": _prize, "idx": _idx})
    g.sendChangeInfo(conn, _prizeData)
    _resData["prize"] = _prize

    # 设置领奖标记
    g.m.shilianztfun.setFoeverTaskRec(uid, _idx)
