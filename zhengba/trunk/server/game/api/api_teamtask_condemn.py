...
def doproc(conn,data):
    ...
    _boss = g.mdb.find1('gonghuiattr', {'ghid': _ghid, 'ctype': 'teamtask_leader', 'k':{'$exists':1}}, sort=[['k', -1]],fields=['_id','ctime','k','ispass'])
    # boss已存在
    ...
    if int(_leaderId) > len(_con['boss']):
        _leaderId = str(len(_con['boss']))
