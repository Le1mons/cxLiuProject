...
def proc(conn, data):
    ...
    :param conn:
    :param data: [玩家uid:str, 阵容:dict]
    :return:
    ::

        {'d': {'fightres':[{fightres}],
                'jifenchange': {'rivaljifen':对手积分,'add':增加的,'reduce':减少的,'jifen':我的积分}
        }
        's': 1}
...
def doproc(conn, data):
    ...
    # 获取之前的缓存
    _teamData = g.mc.get('crosswz_daluandou_{}'.format(uid)) or []
    _teamData.append(data[1])
    if len(_teamData) < 3:
        g.mc.set('crosswz_daluandou_{}'.format(uid), _teamData, 60)
        return _res

    g.mc.delete('crosswz_daluandou_{}'.format(uid))
    heroData = _teamData[-3:]
    ...
    # 获取玩家报名信息
    _userData = g.crossDB.find('wzbaoming', {'uid': uid, 'dkey': _dkey}, fields=['_id', 'uid', 'jifen', 'touid', 'zhanli', 'fightdata', 'openday'])
    # 验证是否报名
    if len(_userData) <= 0:
        # 未报名
        _res['s'] = -4
    ...
    for index in xrange(3):
        ...
        if fightRes['winside'] == -2:
            fightRes['winside'] = int(_myZhanli <= _toHero['zhanli'])
    ...
    _data = {'fightdata': _myData['fightdata'], 'curzhanli': _myZhanli, 'sid':str(g.getSvrIndex())}
    ...
    # 获取随机报名号
    _toUid = g.m.crosswzfun.getRandomUid(_myData['uid'], ugid, groupList)
    if _toUid == None:
        # 寻找对手失败
        _res['s'] = -7
