...
def _checkCond(uid,data):
    ...
    # 任务信息
    _taskCon = _hdinfo["data"]["openinfo"]["task"]["boxprize"][_idx]
    _taskInfo = g.m.wangzhezhaomufun.getTaskInfo(uid, _hdinfo)
    if _taskInfo["jifen"] < _taskCon["val"]:
        _chkData['s'] = -2
        _chkData['errmsg'] = g.L('wangzhezhaomu_taskjifenprize_res_-2')
        return _chkData

    # 判断是否领奖
    if _idx in _taskInfo["boxreclist"]:
        _chkData['s'] = -3
...
def doproc(conn, data):
    ...
    _taskInfo["boxreclist"].append(_idx)

    # 设置活动数据
    _setData = {}
    _setData["boxreclist"] = _taskInfo["boxreclist"]
    g.m.wangzhezhaomufun.setTaskInfo(uid, _hdinfo, _setData)

    _prize = _taskCon["prize"]
    # 获取奖励
    _sendData = g.getPrizeRes(uid, _prize, act={'act': 'wangzhezhaomu_taskjifenprize', 'idx': _idx, "prize": _prize})
