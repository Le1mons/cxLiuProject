...
def doproc(conn, data):
    ...
    _recHid = _hdinfo["data"]["openinfo"]["zhaomu"]["mainhero"][_info["choose"]]["prize"][0]["t"]
    _info["rechero"].update({str(_idx): _recHid})

    # 设置活动数据
    _setData = {}
    _setData["reclist"] = _info["reclist"]
    _setData["rechero"] = _info["rechero"]
    g.m.wangzhezhaomufun.setZhaoMuInfo(uid, _hdinfo, _setData)

    _prize = _con["prize"]
    # 获取奖励
    _sendData = g.getPrizeRes(uid, _prize, act={'act': 'wangzhezhaomu_zhaomuboxprize', 'idx': _idx, "prize": _prize})
