...
def doproc(conn, data):
    ...
    g.setAttr(uid, {'ctype': 'worldtree_swap'}, {'hid':_prize['t'],'star':_star, "tid":tid})

    # 锁定该英雄
    # g.mdb.update('hero',{'_id':g.mdb.toObjectId(tid),'uid':uid},{'islock': 1})
    # g.sendChangeInfo(conn, {'hero': {tid: {'islock': 1}}})
    _res['d'] = {'hero': _prize}
    return _res
