...
def doproc(conn, data):
    ...
    g.m.dball.writeLog(uid, 'xstask_shuaxin', map(lambda x:x['color'], _taskList))
    _heroList = list()
    for i in _taskList:
        i['_id'] = str(i['_id'])
        if 'herolist' in i:
    ...
    _res['d'] = {'task': _taskList, 'freenum': _freeNum, 'herolist': _heroList,'rfnum':_rmbRefNum}
    return _res
