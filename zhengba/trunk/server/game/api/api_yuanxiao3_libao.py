...
def _checkCond(uid, data):
    ...
    _con = g.m.yuanxiao3fun.getCon()['libao'][_id]
...
def doproc(conn, data):
    ...
    # # 扣除奖励
    _send = g.delNeed(uid, _need, 0, {'act': 'yuanxiao3hd_libao', 'id': _id})
    g.sendChangeInfo(conn, _send)

    # 设置任务领奖
    g.m.yuanxiao3fun.setData(uid, _chkData['hdid'], {'$inc': {'libao.{}'.format(_id): 1}})
    _send = g.getPrizeRes(uid, _prize, {'act': 'yuanxiao3hd_libao', 'id':_id})
    g.sendChangeInfo(conn, _send)
