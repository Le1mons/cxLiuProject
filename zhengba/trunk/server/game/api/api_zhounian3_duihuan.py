...
def _checkCond(uid, data):
    ...
    _con = g.m.zhounian3fun.getCon()['duihuan'][_id]
    _data = g.m.zhounian3fun.getData(uid, _hd['hdid'])
    # 任务没有完成
...
def doproc(conn, data):
    ...
    # 删除道具
    _send = g.delNeed(uid, _chkData['need'], 0, {'act': 'zhounian3_duihuan', 'id':_id})
    g.sendChangeInfo(conn, _send)

    _prize = g.mergePrize(_prize * _num)
    # 设置任务领奖
    g.m.zhounian3fun.setData(uid, _chkData['hdid'], {'duihuan': _data["duihuan"]})

    _send = g.getPrizeRes(uid, _prize, {'act': 'zhounian3_duihuan', 'id':_id})
    g.sendChangeInfo(conn, _send)
