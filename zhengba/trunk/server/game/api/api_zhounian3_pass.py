...
def _checkCond(uid, data):
    ...
    # 判断终极奖励是否领取
    if not _data["lottery"]["targetrec"]:
        _chkData['s'] = -3
        _chkData['errmsg'] = g.L('global_valerr')
        return _chkData
...
def doproc(conn, data):
    ...
    # 如果有下一层
    # if str(int(_layer) + 1) in _con["lotteryinfo"]:
    _layer = str(int(_layer) + 1)
    _data["lottery"]["layer"] = _layer

    _setData = {}
    # 重置奖励
    _setData["lottery"] = {"gezi": g.m.zhounian3fun.generate(_layer), "rec": {}, "target": -1, "layer":_layer, "extrec":[], "targetrec":0,  "targetrnum":_data["lottery"]["targetrnum"]}
    _data["lottery"] = _setData["lottery"]
