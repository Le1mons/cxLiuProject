...
def proc(arg,karg):
    ...
    # 发送奖励后清空数据
    _allUser = g.mdb.find('playattr', {'ctype': 'friend_jifen', 'k': _dKey}, sort=[['v', -1]], fields=['_id', 'uid', "v"])
    for user in _allUser:
        g.mdb.update('playattr',{"uid": user["uid"], 'ctype':'friend_jifen'},data={'v':0, "last":user["v"]})
