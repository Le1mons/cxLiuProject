...
def proc(arg,karg):
    ...
    # 更新报信息
    _openDay = g.m.crosswzfun.getDOpenDay(_nt + 3600)

    for _id in _idList:
        try:
            g.crossDB.update('wzbaoming', {'_id': _id}, {
                '$set':{'dkey': _dkey, 'ttltime': _ttlTime, 'jifen': 0, 'fightdata': [], 'renum': 0, 'touid': '', 'openday': _openDay,'sid':str(g.getSvrIndex())},
                '$unset':{'ugid': 1}
            })
        except:
            print "wangzhe_autobm_err"
    # 发送邮件
    if _email:
