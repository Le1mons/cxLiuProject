...
def isCanSignUpGame(ghid):
    ...
    # _chkKey = ['guild','fail','after']
    # for sid in g.getSvrList():
    #     sid = str(sid)
    #
    #     for ck in  _chkKey:
    #         _chkGhList = []
    #         if ck in _data and sid in _data[ck]: _chkGhList = map(lambda x:x['ghid'],_data[ck][sid])
    #
    #         #if sid in _data.get('guild',{}): _chkGhList = map(lambda x:x['ghid'],_data['guild'][sid])
    #         if ghid in _chkGhList:
    #             # 不能报名
    #             return 0
    #
    #     # 有匹配对手过
    #     _chkGhList = map(lambda x:x['ghid'],_data.get('matched', []))
    #     if ghid in _chkGhList:
    #         break
    # else:
    #     _res = 1

    _ghidLists = getGhidLists(_data, ghid)
    for i in _ghidLists:
        if ghid in _ghidLists[i]:
            _res = 0
            break

    return _res

# 获取报名 匹配 失败 隔天的ghid列表
def getGhidLists(_data, ghid):
    _res = {'guild': [], 'fail': [], 'after': [], 'matched': []}
    for sid in g.getSvrList():
        sid = str(sid)

        for ck in  _res:
            if ck == 'matched':
                for i in _data.get('matched', []):
                    if i['ghid'] == ghid:
                        _res[ck].append(ghid)
                        break
            else:
                _chkGhList = []
                if ck in _data and sid in _data[ck]:
                    for i in _data[ck][sid]:
                        if i['ghid'] == ghid:
                            _chkGhList.append(ghid)
                            break
                _res[ck] += _chkGhList
...
def getCompetingData(ghid):
    ...
    if ghid in _ghidLists['fail'] and g.C.HOUR() >= 6:
        _res['status'] = 3
    ...
    elif ghid not in _ghidLists['guild'] and ghid not in _ghidLists['matched'] and ghid not in _ghidLists['after'] and ghid not in _ghidLists['fail']:
        _res['status'] = 1
        # 公会数量
        _res['guildnum'] = getSignUpGuildNum(_data)
    # 已报名  没有打过的 0到6点  报名没有匹配的  星期天 匹配了0点到6点
    elif g.C.WEEK() == 0 or (ghid not in _data.get('settle', []) and 0<=g.C.HOUR()<6) or ghid not in _ghidLists['matched']:
        _res['status'] = 2
        # 公会数量
        _res['guildnum'] = getSignUpGuildNum(_data)
    # 轮空
    ...
    if not _res:
        ...
        if _user:
            for i in _res:
                for x in _user:
                    if i['uid'] == x['uid']:
                        if 'yj' in x['fightless']:
                            for hero in i['fightdata']:
                                if 'hid' not in hero:
                                    continue
                                if hero['pos'] == x['fightless']['yj']:
                                    hero['pos'] = 7
                                elif hero['pos'] == 7:
                                    hero['pos'] = x['fightless']['yj']
                            i['fightdata'].sort(key=lambda x:x.get('pos',0))
...
def getGuildMembers(season, ghid):
    ...
    for i in _data:
        ...
        # 如果定时器没有成功上传玩家数据 轮空
        # if i['ghid'] not in _res:
        #     _ghidList = map(lambda x:x['ghid'], _data)
        #     g.crossDB.update('competing_main',{'ghid': {'$in':_ghidList}}, {'$unset':{'group':1,'rival_ghid':1},'$set':{'lunkong':1}})
        #     for ghid in _ghidList:
        #         insertSegmentPrize(ghid, _data[0]['segment']+1 if _data[0]['segment']<4 else 4)
        #     _ok = False
        # else:
...
def getXiuzhanTopSegment(season, ghid):
    ...
    if _data:
        ...
        for sid in sids:
            sid = str(sid)
            if sid not in _data['fail']:
                continue
            for i in _data['fail'].get(sid, []):
                if i['ghid'] == ghid:
                    return i.get('segment', 1)
...
def insertSegmentPrize(ghid, segment):
    ...
    if not _data:
        try:
            ...
            print "setattr error---------------", segment,ghid,_dKey
...
def script():
    import random
    # g.mdb.delete('gonghui')
    _all = g.mdb.find('gonghuiuser',{},fields=['_id','uid'])
    g.mdb.update('userinfo', {'lv':{'$lt': 30}}, {'lv':30})
    g.mc.flush_all()
    all = g.mdb.find('userinfo', {'uid':{'$nin':map(lambda x:x['uid'], _all)}}, sort=[['maxzhanli', -1]])
    assert len(all) + len(_all) >= 500
    name = g.mdb.count('gonghui') + 1
    gonghuilist = []
    hidList = [t for t in g.GC['hero']]
    for i in all:
        gonghuilist.append(i['uid'])
        if len(gonghuilist) >= 10:
            a = g.m.gonghuifun.createGonghui(gonghuilist[0], 'guild_{}'.format(name), '1',0, auto=1)
            power = 3
            while gonghuilist:
                uid = gonghuilist.pop(0)
                if not gonghuilist:
                    power = 0
                g.m.gonghuifun.joinGonghui(uid, a, power)
                # 查看竞技场0
                if not g.crossDB.find1('jjcdefhero', {'uid': uid}, fields={'_id': 0, 'lasttime': 0}):
                    heroNum = g.mdb.count('hero', {'uid': uid})
                    if heroNum < 6:
                        # 随机发送12个hero
                        randHid = random.sample(hidList, 6 - heroNum)
                        for hid in randHid:
                            _newHero = g.m.herofun.addHero(uid, hid)
                            _heroData = g.m.herofun.reSetHeroBuff(uid, _newHero['tid'])
                    heroList = g.mdb.find('hero', {'uid': uid}, fields={'_id': 1}, sort=[['lv', -1]], limit=6)
                    fight = {}
                    for i in range(1, 7):
                        fight[str(i)] = str(heroList[i - 1]['_id'])
                    g.m.api_zypkjjc_defend.doproc(g.debugConn, [fight])
            name += 1

    for i in g.mdb.find('gonghuiuser',{'power':0},fields=['_id','uid']):
        g.debugConn.uid = i['uid']
        s = g.m.api_ghcompeting_signup.doproc(g.debugConn, [])
        if s['s'] != 1:
            print s['s']

    g.mdb.delete('gameattr',{'ctype':'ghcompeting_timer'})
    g.m.timer_competing_chkopen.proc(1, 1)
    g.m.timer_competing_upload.proc(1, 1)
    g.m.timer_competing_mirror.proc(1, 1)
    g.m.timer_competing_match.proc(1, 1)


if __name__ == '__main__':
    # gh = g.mdb.find('gonghui',{},sort=[['lv',-1]],limit=100)
    # _season = getSeasonNum()
