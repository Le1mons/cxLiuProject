...
#获取邮件
def getEmailList(uid, keys='',where=None, siliao=0,gm=True,**kwargs):
    # 获得关于我的或者全服邮件(SYSTEM)
    _nt = g.C.NOW()
    if not siliao and gm:
        _where = {"uid": {"$in": [uid, "SYSTEM"]}, 'passtime': {'$gte': _nt}}
    elif not siliao:
        _where = {"uid": {"$in": [uid, "SYSTEM"]}, 'passtime': {'$gte': _nt}, 'from':{'$exists':0}}
    else:
...
def ifSysEmailValid(uid, email):
    ...
    if "needvip" in email:
        if isinstance(email['needvip'], list) or isinstance(email['needvip'], tuple):
            # print 'here'
            if gud['lv'] < email['needvip'][0] or gud['lv'] > email['needvip'][1]:
                _retVal = 0

        else:
            if gud['lv'] < email['needvip']:
                _retVal = 0
    ...
    # 渠道id判断
    if 'ext_owner' in email:
        if 'ext_owner' not in gud or str(gud['ext_owner']) not in email['ext_owner']:
            return 0
...
# 发送跨服工会邮件
def sendGongHuiCrossEmail(ghid, sid, title, content, prize=None, **kwargs):
    _nt = g.C.NOW()
    _tmp = getCrossEmailTemplate(etype=2)
    _tmp['ghid'] = ghid
    _tmp['sid'] = str(sid)
    _tmp['title'] = title
    ...
    _r = g.crossDB.insert('crossemail', _tmp)
    return _r

# 获取跨服邮件的模版
def getCrossEmailTemplate(now=None, ttltime=None, etype=1):
    _tmp = {
        'ctime': now or g.C.NOW(),
        'getprize': 0,
        'ifpull': 0,
        "etype": etype,
        'ttltime': ttltime or g.C.TTL()
    }
    return _tmp
...
# 发送跨服邮件
# 注意sid是字符串形式
def sendCrossEmail(uid, sid, title, content, prize=None, **kwargs):
    _nt = g.C.NOW()
    _tmp = getCrossEmailTemplate()
    _tmp['uid'] = uid
    _tmp['sid'] = str(sid)
    _tmp['title'] = title
    _tmp['content'] = content
