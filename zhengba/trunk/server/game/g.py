...
# fixme 路径优化
_root_path = os.path.dirname(os.path.abspath(__file__)).split('game')[0]

#获取ROOT路径
# ROOTPATH = os.getcwd().split('game')[0]
ROOTPATH = os.path.dirname(os.path.abspath(__file__)).split('game')[0]
print '========',ROOTPATH,'====='
...
def set_override_time(_nt):


    if os.path.isdir('/home/www/default/changetime'):
        val = 0
...
class ModuleManage:
    ...
    def load (self,name):
        if not name in x._MODULEDICT_:
            ...
            if ispypy:x._MODULEDICT_[name].str = _str
            '''except Exception as e:
                print e
                print 'import',name,'error'
                '''
            if hasattr(x._MODULEDICT_[name], 'getHongDian'):
                x.REDPOINT_MODULE.append(name)
...
class List2tuple:
    ...
    def _do (self,d,parent=None,deep=1):
        parent = parent or []
...
class GameConfig:
    ...
    def readJson (self,fileName):
        ...
        #不try直接抛出错误
        #json.loads会在以后每次读取时，有个转utf8的过程，实测读性能比直接exec会慢4倍以上
        #优先采用exec直接转对象，如果因为 null 等导致转失败的话，再采用json.loads
        try:
            exec("_json="+ str(txt))
        except:
    ...
    def getConfig (self,fileName,returnClass=True):
        #读配置并格式化
        fileName = self._fixJsonName(fileName)
        if not fileName in self._cache:
...
def fmtPrizeRes(data):
    ...
    _res =  {}
    _chkKeys = {"attr":{},"item":{},"hero":{},'equip':{},'shipin':{},'glyph':{},'pet':{},'wenwu':{},'wuhun':{}}
    for ele in data:
        _key = ele["t"]
        _val = int(ele["n"])
    ...
    _res['wenwu'] = _chkKeys["wenwu"]
    _res['wuhun'] = _chkKeys["wuhun"]
    _res['pet'] = _chkKeys["pet"]
...
def getPrizeRes(uid,data,act=''):
    ...
    for k,v in _prizeRes["attr"].items():
        ...
        # 竞技场经验
            ...
            _r = m.userfun.altGpjjcExp(uid, v, act)
            _tmpRinfo = {}
            _tmpRinfo.update(_r)
            if "addgpjjcexp" in _r:
                del _r["addgpjjcexp"]
    ...
    for hid,num in _prizeRes["hero"].items():
        ...
        event.emit('trial', uid, '1', cond=_heroCon[hid]['star'],val=num)
        event.emit('herotheme_star', uid, hid, val=_heroCon[hid]['star'])
        # 统御
        event.emit("hero_tongu", uid, hid)
        # 挑战活动-记录英雄星级数据
        _act = act
        if type(act) == dict:
            _act = act["act"]
        if _act not in ("hero_reborn", "rearmy_reborn"):
            # 增加名将绘卷列表
            event.emit("addmjhj", uid, hid, num)
    ...
    # 增加神宠
    for k,v in _prizeRes["pet"].items():
        _jhRes = m.petfun.addPet(uid,k,num=v)
        _res["pet"].update(_jhRes)

    # 增加文物
    for k,v in _prizeRes["wenwu"].items():
        _jhRes = m.yjkgfun.addWenwu(uid,k,num=v)
        _res["wenwu"].update(_jhRes)

    # 增加武魂
    for k,v in _prizeRes["wuhun"].items():
        _jhRes = m.wuhunfun.addWuhun(uid,k,num=v)
        _res["wuhun"].update(_jhRes)
...
def fmtSendPrizeData(uid,pdata):
    ...
    if "item" in pdata and len(pdata["item"])>0:
        ...
        if TidList:
            ...
            _w = {'_id':{'$in': oidList}}
            _itemList = m.itemfun.getItemList(uid, keys='itemid,num,passtime,paymoney,lasttime,usetype,etime,bagtype,color', where=_w)
            for ele in _itemList:
                tid = str(ele['_id'])
                del ele['_id']
                itemMap[tid] = ele
    ...
    # 皮肤
    if 'skin' in pdata and len(pdata['skin']) > 0:
        _tmpData["skin"] = {}
        _tmpData["skin"].update(pdata["skin"])

    # 宠物
    if 'pet' in pdata and len(pdata['pet']) > 0:
        _tmpData["pet"] = {}
        _tmpData["pet"].update(pdata["pet"])

    # 文物
    if 'wenwu' in pdata and len(pdata['wenwu']) > 0:
        _tmpData["wenwu"] = {}
        _tmpData["wenwu"].update(pdata["wenwu"])

    # 武魂
    if 'wuhun' in pdata and len(pdata['wuhun']) > 0:
        _tmpData["wuhun"] = {}
        _tmpData["wuhun"].update(pdata["wuhun"])
...
def setAttr(uid,where,data,istime=1):
    ...
    if 'upserted' in _res:
        mdb.update("playattr", _w, {'ctime': _nt})
    return _res
...
def chkOwnerIs(owner):
    """
    检查渠道

    :param owner: str 渠道名字
    :return:
    """
    if 'OWNER' in conf and (conf['OWNER']).find(owner) != -1:
        return True
    return False
...
def chkOpenCond(uid,gongneng):
    ...
    if 'main' in _con[gongneng]:
        for ele in _con[gongneng]['main']:
            ...
            v = ele[1]
            #单个条件的判断
            if k in ('lv','opencityid','vip', "mapid") and gud[k] < int(v):
                _main = False
                break
            elif k == 'severday' and getOpenDay() < v:
                _main = False
                break
...
#检测删除物品是否足够
def chkDelNeed(uid,data,petTid=None):
    _data = data
    _res = {'res':True}
    _chkAttrKey = {}
    for d in _data:
        ...
        if _a == 'attr':
            ...
            # 金币和钻石重新检测以下
            if _t in ('jinbi', 'rmbmoney', 'jifen'):
                _chkAttrKey[_t] = {'$gte': -_delNum}
            ...
            _chk = [True]
            _shipinInfo = m.petfun.getPetInfo(uid, _t, petTid)
            if _shipinInfo['num'] + _delNum < 0:
                _chk[0] = False
        elif _a == 'wenwu':
            _chk = [True]
            _wenwu = mdb.find1('wenwu',{'uid':uid,'wid':_t},fields=['_id','num']) or {'num':0}
            if _wenwu['num'] + _delNum < 0:
                _chk[0] = False
        else:
    ...
    if _chkAttrKey:
        _chkAttrKey['uid'] = uid
        _r = mdb.find1('userinfo', _chkAttrKey, fields={'_id':1})
        if _r is None:
            _res['res'] = False
            _res['a'] = 'attr'
            _res['t'] = _chkAttrKey.keys()[0]
            return _res
...
#issend:扣除物品后是否推送信息
def delNeed(uid,data,issend=1,logdata=None,petTid=None):
    _data = data
    _res = {}
    _send = {}
    for d in _data:
    ...
    # 删除神宠
    if 'pet' in _res:
        for eid,num in _res['pet'].items():
            _shipinInfo = m.petfun.delPet(uid, eid, num, petTid)
            _send['pet'].update(_shipinInfo)

    # 删除文物
    if 'wenwu' in _res:
        for wid,num in _res['wenwu'].items():
            _shipinInfo = m.yjkgfun.delWenwu(uid, wid, num)
            _send['wenwu'].update(_shipinInfo)
...
#检测是否创建跨服聊天队列
def chkCrossServer(gid):
    _date =C.DATE()
    _chkDate = getCrossServerQueueCache(portIndex)
    #print '_chkDate==============',_chkDate
    if _date == _chkDate:
        return
    
    _sysConfig = conf
    _ver = _sysConfig['CROSSVER']
    _preName = C.STR('cross_{1}_{2}',_ver,gid)
    #print 'createCrossServer=========',_preName
    global crossQueue
    crossQueue = MemcacheQueue(crossMC,_preName)
    #print 'portIndex==============',portIndex
    setCrossServerQueueCache(portIndex)
    
    
#根据portIdx存储跨服聊天更新缓存日期
def setCrossServerQueueCache(portidx):
    _key = C.STR("CROSSSERVERQUERE_{1}",portidx)
    mc.set(_key,C.DATE())
    
#根据portIdx存储跨服聊天获取缓存日期
def getCrossServerQueueCache(portidx):
    _key = C.STR("CROSSSERVERQUERE_{1}",portidx)
    return mc.get(_key)
...
if not hasattr(x,'gInit'):
    ...
    # 公平竞技场监听
    if hasattr(crosschatfun, 'gpjjcQueue'):
        event.on("resetBindOver", crosschatfun.gpjjcQueue.getQueue)

    # 跨服playattr对象
    CROSSATTR = BASEDB(crossDB, 'crossplayattr', 'crossplayattr')

    xLog = xsdk.XData(xsdk.MdbConsumer(mdb.mongodb.get_collection('xlog')),getGud if getSysConfigVER() != 'cross' else m.crosscomfun.getCrossGud)  # 打点数据
    import pathDebug

    toup = xsdk.Mdb2X(mdb.mongodb['xlog'])
