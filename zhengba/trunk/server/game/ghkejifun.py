...
# 判断是否全部满级
def isSkillMax(uid, iscache=True):
    _cacheRank = g.mc.get("ghkjskillmax_{}".format(uid))
    # 默认读取缓存
    if iscache and _cacheRank != None:
        ...
        _keji = _con["keji"]
        _skill = _con["skill"]
        _where = {'ctype':'keji_data'}
        _datas = g.getAttr(uid, _where, isnone=0)
        _maxId = []
        for d in _datas:
            for id in _keji[d["k"]]["skill"]:
                if _skill[id]["maxlv"] > d["v"].get(id, 0):
                    break
            else:
                _maxId.append(d["k"])
        if len(_maxId) >= len(_keji):
            _isMax = True
        g.mc.set("ghkjskillmax_{}".format(uid), _isMax)
    return _isMax
