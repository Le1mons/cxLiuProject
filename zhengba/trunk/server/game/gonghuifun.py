...
def createGonghui(uid, name, flag, notice, lv=1, auto=1):
...
def exitGonghui(uid, ghid, exittype=0):
    ...
    # 删除工会_攻城掠地数据
    _siegeInfo = g.m.gonghuisiegefun.getUserData(uid)
    if _siegeInfo:
        _jifenInfo = _siegeInfo["jifeninfo"]
        for cityid, jifen in _jifenInfo.items():
            g.m.gonghuisiegefun.setGongHhuiCityJiFen(uid, cityid, _ghid, -jifen)
    # 删除数据
    g.crossDB.delete("gonghui_siege", {"uid": uid, "dkey": _dkey})
...
def onGongHuiFuBenChk(ghid, fbid, uid):
    ...
    # 通过inc原子性操作确保唯一性
    _mark = GHATTR.getAttrOne(ghid, {'ctype': 'guild_boss_mark'}, keys='_id,v')
    if _mark:
        _r = GHATTR.setAttr(ghid, {'ctype': 'guild_boss_mark', 'v': _mark['v']}, {'$inc': {'v': 1}})
        if _r['updatedExisting'] == False:
            return
    else:
        GHATTR.setAttr(ghid, {'ctype': 'guild_boss_mark'}, {'$inc': {'v': 1}})
...
def OnChongzhiSuccess(uid, act, money, orderid, payCon):
    ...
    # 记录贡献
    setGuildContri(uid, _ghid, _prize)
...
def getImpeachInfo(gid):
    ...
    if not _data:
        ...
        g.mc.set(g.C.STR('ghimpeach_{1}', gid), _data, 24 * 3600)
    return _data
...
def chkImpeach(ghid):
    ...
    # 超过24小时  ctime不为0
    if _data['ctime'] + 24 * 3600 < g.C.NOW() and _data['ctime'] != 0:
        _prehz = g.mdb.find1('gonghuiuser', {'ghid': ghid, 'power': 0}, fields=['_id', 'uid'])
        _tmpGud = g.getGud(_prehz["uid"])
        _con = getCon()
        if g.C.NOW() < _tmpGud["logintime"] + _con['impeach_time'] or not _data['v']:
...
# 增加公会捐献记录
def setGuildContri(uid, ghid, _p):
    _data = g.getAttrOne(uid, {'ctype':'guild_contribution','k':ghid},keys='_id,v') or {'v':0}
    for i in _p:
        if i['a'] == 'item' and i['t'] == '2003':
            _data['v'] += i['n']
            break

    g.mdb.update('gonghuiuser',{'uid':uid},{'sungongxian': _data['v']})
    g.setAttr(uid, {'ctype': 'guild_contribution'}, {'v': _data['v'],'k':ghid})
...
if __name__ == '__main__':
    uid = g.buid('xuzhao')
    onGongHuiFuBenChk("6099e1634d57c873b9832f30", "2", "70_6099dff24d57c873bccd6e79")