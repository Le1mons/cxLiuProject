...
# 获取key
def getKey(minus=False):
    _day = g.getOpenDay() - 1 if minus else g.getOpenDay()
    _key = None
    for i in sorted(g.GC['meirishouchong']['data'].keys(), key=lambda x:int(x)):
        if _day < int(i):
            break
        _key = i
...
# 是否开启
def chkOpen(uid):
    if g.C.getTimeDiff(g.C.NOW(), g.getGud(uid)['ctime']) + 1 < 4 or g.getGud(uid)['vip'] < 1:
        return False
    return True
