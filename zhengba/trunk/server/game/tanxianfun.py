...
def getTanXianMain(uid):
    ...
    _2h = getGuaJiPrize(uid, 3600*2)
    # 预览奖励
    _res['2h'] = {'exp':0,'jinbi':0,'jifen':0,'useexp':0}
    for i in _2h:
        if i['t'] not in _res['2h']:
            continue
        _res['2h'][i['t']] += i['n']
...
def getXSHDprize(prize, gjData):
    ...
    _htypes = [_xsdlHtype, _jrdlHtype, 57]
    _infos = g.mdb.find('hdinfo',{'htype':{'$in':_htypes},'rtime': {'$gte': _nt},'stime':{'$lte': _nt}},fields=['_id','data'])

    # 查看中秋节活动
    _hd = g.m.huodongfun.getHDinfoByHtype(62, ttype="etime")
    if _hd and 'hdid' in _hd and _hd["rtime"] > _nt:
        _infos += [{'data':{'arr': g.m.midautumnfun.getCon(_hd['data']['con'])['drop']}}]
    ...
    for _info in _infos:
        for i in xrange(len(_info['data']['arr'])):
            ...
            _resNum = 0
            for x in xrange(gjData // _cd):
                if 'rand' in _info['data']['arr'][i] and not g.C.RAND(1000, _info['data']['arr'][i]['rand']):
                    continue
                else:
                    _resNum += 1

            if _resNum > 0:
                prize += [{'a':j['a'],'t':j['t'],'n':j['n']*_resNum} for j in _xsPrize]
    return prize
