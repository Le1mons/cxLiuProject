...
def setFuBenSendEmail(uid, fbid):
    _ctype = 'teamtask_fubenemail'
    g.setAttr(uid, {'ctype': _ctype}, {'$push': {'v': str(fbid)},'$set':{'time.{}'.format(fbid): g.C.NOW()}})

# 删除该玩家的排名
def unsetUserDps(uid):
    _ghid = g.getGud(uid)['ghid']
    # 删除副本的伤害
    g.mdb.update('gonghuiattr', {'ctype':'teamtask_leader','ghid':_ghid},{'$unset': {'uid2dps.{}'.format(uid): 1}})
    # 删除贡献
    g.mdb.delete('playattr', {'ctype':'teamtask_contri','uid':uid})


if __name__ == '__main__':
    gud = g.getGud(g.buid('lsq111'))
    setTeamTaskVal(g.buid('lsq111'), '6')