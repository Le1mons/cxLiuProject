...
def createTask(uid,colorArgs=None):
    ...
    # 保底值
    _guarantee = g.getAttrByCtype(uid, "xstask_guarantee",bydate=False)
    # 如果玩家有特权,就根据传进的参数随机一个
    _con = g.GC['pre_color2xstaskid']
    _loop = [1] * (_taskNum - len(color))
    ...
    for i in _loop:
        ...
        if int(_task['color']) >= 3:
            _guarantee = 0
        else:
            _guarantee += g.GC['xstaskcom']['lucky']

        # 大于100 就添加紫色任务
        if _guarantee >= g.GC['xstaskcom']['maxlucky']:
            color.append('3')
            _guarantee = 0
            _loop.remove(1)
    ...
    g.setAttr(uid,{'ctype':'xstask_guarantee'},{'v':_guarantee})
    ...
    # 趣味成就
    _qwcj = g.mdb.find1('qwcj',{'uid':uid},fields=['_id','nval'])
    if _qwcj and _qwcj['nval'].get('18',0) >= 3 and not _redTask:
        _set = {'$inc':{'data.19':1}}
    else:
        _set = {'$set':{'data.19': 0},'$inc':{'data.18':1}} if _redTask else {'$set':{'data.18': 0},'$inc':{'data.19':1}}
...
def getXstaskHD(uid):
    ...
    _res = 0
    _tasks = g.mdb.find('xstask',{'uid':uid,'isjiequ':1},fields=['_id','ftime'])
    for i in _tasks:
        if g.C.NOW() >= i['ftime']:
            _res = 1
            break
