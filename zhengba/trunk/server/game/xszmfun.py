...
def getHuodongData():
    ...
    if not _hdInfo or _nt >= _hdInfo['etime']:
        _hdInfo = g.mdb.find1('hdinfo', {'htype': _htype, 'etime': {'$gt': _nt}, 'stime': {'$lt': _nt}},
                              fields=['_id', 'etime', 'hdid', 'stime','model','data',"rtime"])
        # 活动已过期
        if not _hdInfo:
...
# 获取排行信息
def getRankData(hdinfo,uid=None,myrank=True):
    _con = hdinfo['data']['rankprize']
    _myRank = -1
    _list = g.mdb.find('hddata',{'hdid':hdinfo['hdid'],'val':{'$gt': 0}},sort=[['val',-1],['lasttime',1]],limit=10,fields=['_id','uid','val'])
    # 按照段位分开
    ...
    if _myRank == -1 and myrank:
        _valInfo = g.m.huodongfun.getMyHuodongData(uid, hdinfo['hdid'], keys='_id,val')
        if _valInfo['val'] >= 500:
            _myRank = g.mdb.count('hddata',{'val':{'$gte':_valInfo['val']},'hdid':hdinfo['hdid']}) + 1
            if _myRank > 50:
                _myRank = -1
...
def getPrizeByNum(num, idx):
    ...
    for i in xrange(num):
        ...
        if idx == _con['special']:
            ...
            _prize = getHuodongData()['data']['hero'] if not _item['isdlz'] else (
...
def timer_sendPrize(_htype):
    ...
    _resList = []
    _hdid = _info['hdid']
    _rank = getRankData(_info, myrank=False)['ranklist']
    # 补发未领取的目标奖励
    _uid2prize = sendJifenPrize(_info)
    _title = _con['email']['title']
    for i in _rank:
        if not i:
            continue
        _content = g.C.STR(_con['email']['content'], i['rank'])
        _prize = getPrizeByRank(i['rank'], _info['data']['rankprize'])
...
# 获取uid对应的奖励
def sendJifenPrize(info):
    _title = g.GC['xianshi_zhaomu']['email1']['title']
    _allData = g.mdb.find('hddata', {'hdid': info['hdid']}, fields=['_id', 'val', 'gotarr', 'uid'])
    for i in _allData:
        _prize = []
        _canRec = filter(lambda x:i['val']>=x[0], info['data']['jifen2prize'])
        _noRec = set(range(len(_canRec))) - set(i['gotarr'])
        for idx in _noRec:
            _prize += info['data']['jifen2prize'][idx][1]
        if _prize:
            _content = g.C.STR(g.GC['xianshi_zhaomu']['email1']['content'], i['val'])
            g.m.emailfun.sendEmails([i['uid']], 1, _title, _content, g.fmtPrizeList(_prize))



if __name__ == '__main__':
    # print getRankData(1547877286)
    print sendJifenPrize(g.mdb.find1('hdinfo',{'htype':31}))