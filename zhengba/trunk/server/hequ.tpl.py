...
class HeQu:
    ...
    def hq_clearDeadUser(self):
        ...
        _delUserList = self.mdb1.find('userinfo',where={'payexp':{'$exists':0},"lv":{"$lt":60},"logintime":{"$lt":_ptime}},fields={"uid":1,"name":1,"_id":0})
        print w("开始清空死号玩家")
    ...
    # 处理gm邮件
    def hq_emailAction(self):
        print "start hq_emailAction"
        _nt = g.C.NOW()
        _eW = {
            "etype": 1,
            "plist": {"$exists": 1},
        ...
        _emailList = self.mdb1.find("email", _eW, sort=[["sid", 1]])

        # 没有gm邮件或者只有1封
        if not _emailList or len(_emailList) == 1:
            return

        _title2e = {}
        # 按标题passtime分类
        for e in _emailList:
            _eFlag = e["title"] + str(e["passtime"])
            _title2e[_eFlag] = _title2e.get(_eFlag, []) + [e]

        _chkKey = ["plist", "rlist", "dlist"]
        # 合并其他邮件的plist，rlist，dlist
        for elist in _title2e.values():
            if len(elist) == 1:
                continue
            ...
            for e in elist:
                for key in _chkKey:
                    _saveEmail[key] = _saveEmail.get(key, []) + e.get(key, [])
                self.mdb1.delete("email", {"_id": e["_id"]})

            self.mdb1.update("email", {"_id": _saveEmail["_id"]}, _saveEmail)
        print "end hq_emailAction"
