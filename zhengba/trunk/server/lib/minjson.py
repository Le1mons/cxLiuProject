...
#replace chinese word in json string   by ciniao
def replace_cn(s):
	got = s.find('\u',0)
	while got!=-1:
		v = s[got+2:got+6]
...
def safeRead(aString,*fmtChinese):
	...
	if len(fmtChinese)>0 and fmtChinese[0]==True:
		aString = replace_cn(aString)
